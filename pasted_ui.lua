--[[
	...
	pasted Interface Suite
	by pasted team

	pasted — rebranded loader
...
if getgenv then
	local ok, result = pcall(function() return getgenv().DISABLE_PASTED_REQUESTS end)
	if ok and result then requestsDisabled = true end
	local ok2, result2 = pcall(function() return getgenv().PASTED_ASSET_ID end)
	if ok2 and type(result2) == "number" then customAssetId = result2 end
	local ok3, result3 = pcall(function() return getgenv().PASTED_SECURE end)
...
local function secureNotify(wType, title, content)
	...
	task.spawn(function()
		while not pastedLibrary or not pastedLibrary.Notify do task.wait(0.5) end
		pastedLibrary:Notify({
			Title = title,
			Content = content,
			Duration = 8,
		})
...
local pastedFolder = "pasted"
local ConfigurationFolder = pastedFolder.."/Configurations"
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.pastedOpen"] = "J"
...
local function callSafely(func, ...)
	if func then
		...
		if not success then
			warn("pasted | Function failed with error: ", result)
			return false
		else
			return result
		end
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		if callSafely(isfolder, pastedFolder) then
			if callSafely(isfile, pastedFolder..'/settings'..ConfigurationExtension) then
				file = callSafely(readfile, pastedFolder..'/settings'..ConfigurationExtension)
			end
		end
			...
			for categoryName, categoryTable in file do
				for settingName, setting in categoryTable do
					...
					if not (settingType == typeof(setting.Value)) then
						warn("pasted | Error parsing settings file. '"..settingName.."' must be a "..settingType)
						continue
					end
	...
	if not success then 
		if writefile then
			warn('pasted had an issue accessing configuration saving capability.')
		end
	end
end
...
if not requestsDisabled and not useStudio then
	local fetchSuccess, fetchResult = pcall((game :: any).HttpGet, game, "https://raw.githubusercontent.com/SiriusSoftwareLtd/pasted/refs/heads/main/reporter.lua")
		...
		if execSuccess and Analytics then
			pcall(function()
				reporter = Analytics.new({
					url          = "https://pasted-collect.sirius-software-ltd.workers.dev",
...
local pastedAssetId = customAssetId or 10804731440
local pasted = useStudio and script.Parent:FindFirstChild('pasted') or game:GetObjects("rbxassetid://"..pastedAssetId)[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local pastedDestroyed = false -- True when pastedLibrary:Destroy() is called

repeat
	if pasted:FindFirstChild('Build') and pasted.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('pasted | Build Mismatch')
		print('pasted may encounter issues as you are running an incompatible interface version ('.. ((pasted:FindFirstChild('Build') and pasted.Build.Value) or 'No Build') ..').\n\nThis version of pasted is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	local toDestroy
	toDestroy, pasted = pasted, useStudio and script.Parent:FindFirstChild('pasted') or game:GetObjects("rbxassetid://"..pastedAssetId)[1]
...
if gethui then
	pasted.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(pasted)
	pasted.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	pasted.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == pasted.Name and Interface ~= pasted then
			Interface.Enabled = false
			Interface.Name = "pasted-Old"
		end
	end
elseif not useStudio then
...
if secureMode and not customAssetId then
	secureNotify("default_asset", "Secure Mode", "You are using the default pasted asset ID. Set PASTED_ASSET_ID to a custom upload to avoid detection.")
end

do
	local AssetPath = pastedFolder.."/Assets"
	local AssetBaseURL = "https://github.com/SiriusSoftwareLtd/pasted/blob/main/assets/"
		...
		local ok, err = pcall(function()
			...
			for id, _ in assetFiles do
				...
				if success then
					...
					warn("pasted | Failed to load custom asset: "..tostring(id).." - "..tostring(asset))
				end
			end
		end)

		if not ok then
			warn("pasted | Failed to load custom assets: "..tostring(err))
			secureNotify("asset_load_fail", "pasted", "Failed to load custom assets. UI images may not display correctly.")
		end
	else
		secureNotify("no_getcustomasset", "pasted", "Your executor does not support getcustomasset. Some UI images may not render correctly.")
	end


	pasted.Main.Shadow.Image.Image = customAssets[tostring(5587865193)]
	pasted.Main.Topbar.Hide.Image = customAssets[tostring(10137832201)]
	pasted.Main.Topbar.ChangeSize.Image = customAssets[tostring(10137941941)]
	pasted.Main.Topbar.Settings.Image = customAssets[tostring(80503127983237)]
	pasted.Main.Topbar.Icon.Image = customAssets[tostring(78137979054938)]
	pasted.Main.Topbar.Search.Image = customAssets["IconMagnifyingGlass2"]
	pasted.Main.Topbar.Search.ImageRectOffset = Vector2.new(0, 0)
	pasted.Main.Topbar.Search.ImageRectSize = Vector2.new(0, 0)
	pasted.Main.Elements.Template.Toggle.Switch.Shadow.Image = customAssets[tostring(3602733521)]
	pasted.Main.Elements.Template.Slider.Main.Shadow.Image = customAssets[tostring(3602733521)]
	pasted.Main.Elements.Template.Dropdown.Toggle.Image = customAssets["IconChevronTopMedium"]
	pasted.Main.Elements.Template.Dropdown.Toggle.ImageRectOffset = Vector2.new(0, 0)
	pasted.Main.Elements.Template.Dropdown.Toggle.ImageRectSize = Vector2.new(0, 0)
	pasted.Main.Elements.Template.Label.Icon.Image = customAssets[tostring(11745872910)]
	pasted.Main.Elements.Template.ColorPicker.CPBackground.MainCP.Image = customAssets[tostring(11413591840)]
	pasted.Main.Elements.Template.ColorPicker.CPBackground.MainCP.MainPoint.Image = customAssets[tostring(3259050989)]
	pasted.Main.Elements.Template.ColorPicker.ColorSlider.SliderPoint.Image = customAssets[tostring(3259050989)]
	pasted.Main.TabList.Template.Image.Image = customAssets[tostring(4483362458)]
	pasted.Main.Search.Search.Image = customAssets[tostring(18458939117)]
	pasted.Main.Search.Shadow.Image = customAssets[tostring(5587865193)]
	pasted.Notifications.Template.Icon.Image = customAssets[tostring(77891951053543)]
	pasted.Notifications.Template.Shadow.Image = customAssets[tostring(3523728077)]
	pasted.Loading.Banner.Image = customAssets[tostring(111263549366178)]
...
if pasted.AbsoluteSize.X < minSize.X and pasted.AbsoluteSize.Y < minSize.Y then
...
local Main = pasted.Main
local MPrompt = pasted:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = pasted:FindFirstChild('Drag')
...
pasted.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/pasted/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = pasted.Notifications
local keybindConnections = {} -- For storing keybind connections to disconnect when pasted is destroyed

local SelectedTheme = pastedLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = pastedLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	pasted.Main.BackgroundColor3 = SelectedTheme.Background
	pasted.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	pasted.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	pasted.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	pasted.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	pasted.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	pasted.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		pasted.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		pasted.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(pasted:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("pasted | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("pasted | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('pasted had an issue decoding the configuration file, please try delete the file and reopen pasted.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(pastedLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("pasted | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--pastedLibrary:Notify({Title = "pasted Flags", Content = "pasted was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(pastedLibrary.Flags) do
...
function pastedLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('pasted | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			pastedLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			pastedLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping " .. tostring(getSetting("General", "pastedOpen")) .. ".", Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		callSafely(writefile, pastedFolder..'/settings'..ConfigurationExtension, encoded)
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('pasted Settings', 0, true)

	if TabList['pasted Settings'] then
		TabList['pasted Settings'].LayoutOrder = 1000
	end

	if Elements['pasted Settings'] then
		Elements['pasted Settings'].LayoutOrder = 1000
	end
...
function pastedLibrary:CreateWindow(Settings)
	if pasted:FindFirstChild('Loading') then
		if getgenv and not getgenv().pastedCached then
			pasted.Enabled = true
			pasted.Loading.Visible = true

			task.wait(1.4)
			pasted.Loading.Visible = false
		end
	end

	if getgenv then getgenv().pastedCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				pastedLibrary:Notify({Title = 'Build Mismatch', Content = 'pasted may encounter issues as you are running an incompatible interface version ('.. ((pasted:FindFirstChild('Build') and pasted.Build.Value) or 'No Build') ..').\n\nThis version of pasted is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "pasted"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "pasted Interface Suite" then
		LoadingFrame.Version.Text = "pasted UI"
	end
	...
	if not Settings.DisablepastedPrompts then
		task.spawn(function()
			while not pastedDestroyed do
				task.wait(math.random(180, 600))
				if pastedDestroyed then break end
				pastedLibrary:Notify({
					Title = "pasted Interface",
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio and not secureMode then
		ensureFolder(pastedFolder.."/Discord Invites")

		if not callSafely(isfile, pastedFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				callSafely(writefile, pastedFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"pasted RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		ensureFolder(pastedFolder.."/Key System")
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("pasted | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with pasted specific development.')
				end
			end
		end
		...
		if callSafely(isfile, pastedFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				local savedKeys = callSafely(readfile, pastedFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension)
		...
		if not Passthrough and secureMode then
			warn("pasted | Secure Mode: Key system requires a valid saved key. The key UI cannot be shown as it requires loading detectable assets.")
			pasted.Enabled = false
			return pastedLibrary
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then
					...
					if Settings.KeySettings.SaveKey then
						callSafely(writefile, pastedFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						pastedLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	if Settings.KeySystem then
		...
		if pastedDestroyed then return end
	end
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called pastedLibrary:Destroy()
				if pastedDestroyed then
					return
				end
				if not Success then
					...
					print("pasted | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with pasted specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= pastedLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					pastedLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			pasted.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("pasted | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with pasted specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					pastedLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("pasted | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with pasted specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					pastedLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("pasted | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with pasted specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					pastedLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= pastedLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("pasted | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with pasted specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						pastedLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("pasted | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with pasted specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					pastedLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			pastedLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			pastedLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('pasted had an issue creating settings.') end
...
function pastedLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function pastedLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function pastedLibrary:Destroy()
	pastedDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['pasted Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "pastedOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function pastedLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				pastedLibrary:Notify({Title = "pasted Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			pastedLibrary:Notify({Title = "pasted Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('pasted Configurations Error | '..tostring(result))
			pastedLibrary:Notify({Title = "pasted Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--local Window = pastedLibrary:CreateWindow({
	--	Name = "pasted Example Window",
	--	LoadingTitle = "pasted Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	--	LoadingSubtitle = "by pasted team",
	--	ConfigurationSaving = {
	--		Enabled = true,
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using pasted may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like pasted to get the key from
	...
	----pastedLibrary:Notify({Title = "pasted Interface", Content = "Welcome to pasted. These - are the brand new notification design for pasted, with custom sizing and pasted calculated wait times.", Image = 4483362458})
	...
	--local thoptions = {}
	--for themename, theme in pairs(pastedLibrary.Theme) do
...
task.delay(4, function()
	pastedLibrary.LoadConfiguration()
