...
-- ============================================================
-- WEAPON SKINS PANEL (right column, directly below movement)
-- Clean panel: open / close / hotkey only (no extra header text)
-- ============================================================
local WeaponSkinsPanel = Tabs.Character:AddRightGroupbox('weapon skins')
WeaponSkinsPanel:AddButton('open skin changer', function()
    local opened = false
    pcall(function()
        if _G.RivalsAPI and _G.RivalsAPI.OpenMenu then
            _G.RivalsAPI.OpenMenu()
            opened = true
        elseif _G.OpenRivalsSkinChanger then
            _G.OpenRivalsSkinChanger()
            opened = true
        end
    end)
    if not opened then
        Library:Notify('skin changer still loading...', 3)
    else
        -- Remind unlock if hooks not applied yet
        if not _G.rivalsSCHooked then
            Library:Notify('open misc tab → unlock skin changer to apply skins in-game', 4)
        end
    end
end)
WeaponSkinsPanel:AddButton('close skin changer', function()
    pcall(function()
        if _G.RivalsAPI and _G.RivalsAPI.CloseMenu then
            _G.RivalsAPI.CloseMenu()
        elseif _G.CloseRivalsSkinChanger then
            _G.CloseRivalsSkinChanger()
        end
    end)
end)
WeaponSkinsPanel:AddLabel('hotkey: K  |  unlock in misc tab')
...
-- ============================================================
-- INTEGRATED RIVALS SKIN CHANGER (Instance-styled UI)
-- Tabs: Skins / Wraps / Charms / Finishers / Misc
-- Open: character > weapon skins > open skin changer  |  K
-- Unlock: skin changer → misc → Unlock Skin Changer
-- ============================================================
task.spawn(function()
    local src = [==[
-- Rivals Skin Changer v4.1.4 (All Cosmetics Working + Finisher Remote + Misc Unlock)
-- K = toggle skin menu  |  RShift = toggle utils menu
-- AutoExecute safe: guards by UniverseId so it only runs in Rivals
-- 
-- FEATURES:
-- - Skins tab: Apply skins to individual weapons with preview images
-- - Wraps tab: Apply wraps to individual weapons with preview images
-- - Charms tab: Apply charms to individual weapons with preview images (per-weapon selection)
-- - Finishers tab: Apply finishers to individual weapons with preview images (per-weapon selection)
-- - Misc tab: Unlock Skin Changer button (must unlock before hooks apply; re-apply after teleport)
-- - All items show rarity colors and badges
-- - Search filters for all tabs
-- - Config save/load now includes per-weapon wraps, charms, and finishers

-- ============================================================
-- PLACE GUARD  (Rivals universe = 6035872082)
-- Known place IDs:
--   17625359962      = Lobby
--   117398147513099  = 1v1 Matchmaking server
-- ============================================================
local RIVALS_UNIVERSE = 6035872082
local RIVALS_LOBBY_ID = 17625359962
local RIVALS_DUEL_ID  = 117398147513099

local universeId = 0
pcall(function() universeId = game.GameId end)
if universeId ~= RIVALS_UNIVERSE then
    return  -- not Rivals - bail silently
end

local currentPlaceId = game.PlaceId

-- ============================================================
-- STARTUP RESTORE  (runs before anything else)
-- If a previous execution patched ClientViewModel but the Lua VM
-- was torn down before emergencyShutdown could restore it, the
-- module table still holds a dead closure from the old VM.
-- When the new place loads and calls ClientViewModel.new it hits
-- freed memory → crash on the loading screen.
-- This block finds the module directly and restores it NOW.
-- ============================================================
pcall(function()
    -- Method 1: use stored _G refs if they survived
    if _G.rivalsOrigNew and _G.rivalsCVMRef then
        _G.rivalsCVMRef.new = _G.rivalsOrigNew
    end
    if _G.rivalsOrigGetWrap and _G.rivalsCVMRef then
        _G.rivalsCVMRef.GetWrap = _G.rivalsOrigGetWrap
    end
end)
-- Method 2 removed: requiring ClientViewModel too early crashes the game's own scripts
-- because its dependencies (like PlayerDataController) haven't initialized yet.
-- Clear all patch refs so this session starts completely clean
_G.rivalsOrigNew     = nil
_G.rivalsOrigGetWrap = nil
_G.rivalsCVMRef      = nil


_G.rivalsActive       = true
_G.rivalsShuttingDown = false
...
-- Wait for LocalPlayer to exist (crucial for auto-execute)
local player = Players.LocalPlayer
while not player do
    task.wait()
...
if not playerGui then
    warn("[Rivals SC] PlayerGui failed to load in time.")
    return
end

-- ============================================================
-- EMERGENCY SHUTDOWN
-- Must be synchronous and instant. The Lua VM is destroyed at the
-- C++ level when Roblox teleports to a different PlaceId (reserved
-- server). Any thread still running at that moment causes an access
-- violation. The only safe fix is to disconnect everything BEFORE
-- the engine begins the transition, not after.
-- ============================================================
local function emergencyShutdown()
    if _G.rivalsShuttingDown then return end  -- idempotent
    _G.rivalsShuttingDown = true
    _G.rivalsActive       = false
    _G.rivalsSCHooked     = false

    -- Restore monkey-patched closures first.
    -- A patched function holds upvalue refs into THIS Lua VM's stack.
    -- If the module table still holds our closure when the VM is torn
    -- down, the next caller in the new place hits freed memory -> crash.
    pcall(function()
        if _G.rivalsOrigGetWrap and _G.rivalsCVMRef then
            _G.rivalsCVMRef.GetWrap = _G.rivalsOrigGetWrap
        end
    ...
    -- Disconnect every connection. This is the critical step —
    -- a connected Heartbeat/InputBegan that fires mid-VM-teardown crashes.
    local function nukeConns(t)
        if type(t) ~= "table" then return end
        for i = 1, #t do
            pcall(function() t[i]:Disconnect() end)
            t[i] = nil
        end
    end
    nukeConns(_G.rivalsSkinConns)
    nukeConns(_G.rivalsUtilConns)

    pcall(function()
        if _G.rivalsESPHighlights then
            for _, h in pairs(_G.rivalsESPHighlights) do
                pcall(function() h:Destroy() end)
            end
            _G.rivalsESPHighlights = {}
        end
    end)
    pcall(function()
        if _G.rivalsAimHL then
            pcall(function() _G.rivalsAimHL:Destroy() end)
            _G.rivalsAimHL = nil
        end
    end)
    pcall(function()
        ...
        for _, name in ipairs({"__rc1","__rc2","__rivalsACBypassToast","RivalsSkinChanger","WaveSkinChanger","RivalsUtils","InstanceSkinChanger"}) do
            local g = pg:FindFirstChild(name)
            if g then pcall(function() g:Destroy() end) end
        end
    end)
    print("[Rivals SC] Shutdown complete.")
end

-- ============================================================
-- TELEPORT INTERCEPT
-- Use OnTeleport event only — hookfunction on TeleportAsync can
-- corrupt the function on some executors, crashing when matchmaking
-- calls it internally.
-- ============================================================
pcall(function()
...
-- Fallback B: PlayerRemoving
pcall(function()
    Players.PlayerRemoving:Connect(function(p)
        if p == player then emergencyShutdown() end
    end)
end)

-- Fallback C: BindToClose (only register once — not disconnectable)
if not _G.rivalsBindToCloseSet then
    _G.rivalsBindToCloseSet = true
    pcall(function() game:BindToClose(emergencyShutdown) end)
end


-- ============================================================
-- CLEANUP  (re-execution safe — clears any previous run's state)
-- ============================================================
-- Run emergency shutdown to cleanly kill any previous execution
-- before we rebuild everything fresh
emergencyShutdown()
-- Re-init the connection tables fresh
_G.rivalsSkinConns     = {}
_G.rivalsUtilConns     = {}
_G.rivalsESPHighlights = {}
_G.rivalsAimHL         = nil
-- Re-enable for this run
_G.rivalsActive        = true
_G.rivalsShuttingDown  = false
...
["Burst Rifle"] = {"Default", "Electro Rifle", "Aqua Burst", "FAMAS", "Spectral Burst", "Pine Burst", "Pixel Burst", "Glorious Burst Rifle", "Keyst Rifle"},
...
["Flamethrower"] = {"Default", "Pixel Flamethrower", "Lamethrower", "Glitterthrower", "Jack O'Thrower", "Snowblower", "Keythrower", "Rainbowthrower", "Extinguisher", "Glorious Flamethrower"},
...
["Gunblade"] = {"Default", "Hyper Gunblade", "Crude Gunblade", "Gunsaw", "Boneblade", "Elf's Gunblade", "Keyblade", "Glorious Gunblade"},
...
["Daggers"] = {"Default", "Aces", "Paper Planes", "Shurikens", "Bat Daggers", "Cookies", "Crystal Daggers", "Keynais", "Broken Hearts", "Toaster", "Glorious Daggers"},
["Energy Pistols"] = {"Default", "Void Pistols", "Hydro Pistols", "Soul Pistols", "New Year Energy Pistols", "Apex Pistols", "Hacker Pistols", "Hyperlaser Guns", "Glorious Energy Pistols"},
...
["Handgun"] = {"Default", "Blaster", "Hand Gun", "Gumball Handgun", "Pumpkin Handgun", "Gingerbread Handgun", "Pixel Handgun", "Towerstone Handgun", "Warp Handgun", "Glorious Handgun"},
...
["Slingshot"] = {"Default", "Stick", "Goalpost", "Harp", "Boneshot", "Reindeer Slingshot", "Lucky Horseshoe", "Keyshot", "Glorious Slingshot"},
["Spray"] = {"Default", "Lovely Spray", "Nail Gun", "Spray Bottle", "Boneclaw Spray", "Pine Spray", "Key Spray", "Glorious Spray"},
["Uzi"] = {"Default", "Water Uzi", "Electro Uzi", "Money Gun", "Demon Uzi", "Pine Uzi", "Arch Uzi", "Keyzi", "Glorious Uzi"},
["Warper"] = {"Default", "Glitter Warper", "Arcane Warper", "Hotel Bell", "Experiment W4", "Frost Warper", "Electropunk Warper", "Glorious Warper"},
["Battle Axe"] = {"Default", "The Shred", "Ban Axe", "Cerulean Axe", "Mimic Axe", "Nordic Axe", "Balloon Axe", "Street Sign", "Keyttle Axe", "Glorious Battle Axe"},
["Chainsaw"] = {"Default", "Blobsaw", "Handsaws", "Mega Drill", "Buzzsaw", "Festive Buzzsaw", "Glorious Chainsaw"},
["Fists"] = {"Default", "Boxing Gloves", "Brass Knuckles", "Fists of Hurt", "Pumpkin Claws", "Festive Fists", "Fist", "Spy Gloves", "Glorious Fists"},
["Katana"] = {"Default", "Saber", "Lightning Bolt", "Stellar Katana", "Evil Trident", "New Year Katana", "Keytana", "Arch Katana", "Crystal Katana", "Pixel Katana", "Glorious Katana", "Linked Sword"},
["Knife"] = {"Default", "Chancla", "Karambit", "Balisong", "Machete", "Candy Cane", "Keylisong", "Keyrambit", "Caladbolg", "Pencil", "Glorious Knife"},
...
["Grenade"] = {"Default", "Whoopee Cushion", "Water Balloon", "Dynamite", "Soul Grenade", "Jingle Grenade", "Frozen Grenade", "Keynade", "Cuddle Bomb", "Glorious Grenade"},
...
["Satchel"] = {"Default", "Advanced Satchel", "Notebook Satchel", "Bag o' Money", "Potion Satchel", "Suspicious Gift", "Pizza Box", "Glorious Satchel"},
...
["Grappler"] = {"Default", "Lasso", "Glorious Grappler"},
["Spear"] = {"Default", "Giant Pencil", "Studio Light", "Glorious Spear"},
}


-- ============================================================
-- WRAP LIST  (kept for reference, unused at runtime)
-- ============================================================
local _WrapList = {"None", ".dll", ".exe", "1B Visits", "2025 Wrapping", "2026 Wrapping", "A5", "Accretion", "Aegis", "Amber", "Ancient", "Antimatter", "Arabesque", "Arbiter", "Arcane", "Arctic Camo", "Arena", "Aurora", "Aurum", "Beach", "Bee", "Beggar", "Beige", "Birthday Wrapping", "Black", "Black Damascus", "Black Glass", "Black Granite", "Black Opal", "Blaze", "Bliss", "Blizzard", "Blue", "Bluesteel", "Blush", "Blush Wrapping", "Bombastic", "Boomore", "Borealis", "Brain", "Brianore", "Bright", "Brimstone", "Brown", "Bubbles", "Bunsen", "Cactus", "Candlelight", "Candy Apple", "Caned Wrapping", "Carbon Fiber", "Carbon Wrapping", "Cardboard", "Cardinal", "Carmine", "Carpet", "Cartoony Paint", "Cashmere Wrapping", "Celestial", "Celtic", "Cerulean", "Cheese", "Chilled Wrapping", "Chromatic", "Chrome", "Chrome Webs", "Circuit", "Clamshell", "Classic", "Clouds", "Clovers", "Cold Metal", "Community", "Cool", "Cool Crochet", "Copper", "Cork", "Cream", "Creme Wrapping", "Crime Scene", "Crimson", "Crimson Art", "Crossed", "Crystallized", "Cursed", "Damascus", "Danger", "Dark", "Dark Arena", "Dark Matter", "Dawn", "Desert Camo", "Devourer", "Diamond", "Digital Camo", "Disco", "Dunes", "Dusky Wrapping", "Eco", "Egg Fried Rice", "Empress", "Encroached", "Encrypt", "Eruption", "Evergreen Wrapping", "Experience", "Facility", "Fame Stars", "Festive Lights", "Fiery", "Fire", "Fire Horse", "Forest Camo", "Forest Wrapping", "Fortune Wrapping", "Fracture", "Frankenstein", "Frigid", "Frostbite", "Frosted", "Frosty Wrapping", "Galvanized", "Geometric", "Gingerbread", "Glacier", "Glamour", "Glass", "Glisten", "Glorious", "Glossy", "Gold", "Grass", "Green", "Green Goo", "Green Sparkle", "Greenflame", "Groove", "Gunmetal", "Hallow", "Hammered Copper", "Haunted", "Heartfelt", "Heirloom", "Hesper", "Highlighter", "Holly Wrapping", "Hologram", "Hologram Arena", "Honey", "Honeycomb", "Hot Cocoa", "Hyperdrive", "Hypnotic", "Ice Queen", "Igneous", "Indigo Sparkle", "Indigo Wrapping", "Inlets", "Insidious", "Insignia", "Invisible", "Iridescent", "Jean", "Jolly Wrapping", "Ladybug", "Lapis", "Lavish Crystal", "Leafy Grass", "Lemon", "Lightning", "Limefire", "Liquid Chrome", "Liquid Gold", "Lovely Leopard", "Lucid Halftone", "Lucre", "Lumber", "Luxe", "Luxurious", "Luxury Wrapping", "MaGGenta", "Machine", "Magma", "Magnetite", "Mahogany", "Mainframe", "Maize", "Malachite", "Malevolent", "Maroon", "Medium stone grey", "Merlot Wrapping", "Mesa", "Mesh", "Messis", "Midnight", "Mint", "Mint Choco Chip", "Mintbread", "Minty Wrapping", "Mischief", "Mocha Wrapping", "Model", "Money", "Moonstone", "Mossy Stone", "Mousse Wrapping", "Mummy", "Mustard", "Navy", "Nebula", "Necromancer", "Nekore", "Neo", "Neon Lights", "Net", "Noir", "Normal", "Nosnite", "Nova", "Obsidian", "Ocean Camo", "Olive", "Olo", "Omnisand", "Only Six", "OranGG", "Orange", "Orichalcum", "Ornamented", "Ornate", "PB & J", "Paint", "Paparazzi", "Patriot", "Pearly Wrapping", "Peppermint", "Peppermint Wrapping", "Peridot", "Peril", "Periwinkle Wrapping", "Pine", "Pink", "Pink Crochet", "Pink Glitter", "Pink Lemonade", "Pixel Blight", "Pixel Light", "Plaid Pajama", "Plasma", "Plastic", "Playful", "Polar Wrapping", "Polaris", "Popcorn", "Portal", "Purple", "Purple Goo", "Purpleize", "Quartz", "Quasar", "RIVALS Wrapping", "Rage", "Red", "Red Carpet", "Red Rubber", "Regal", "Regal Wrapping", "Reptile", "Resolute", "Rift", "Rivalry", "Rug", "Ruined Pillars", "Rust", "Rustic", "Salmon", "Scales", "Scorched", "Scourge", "Scribble", "Sensite", "Sentinel", "Serenity", "Shadore", "Shadow Halftone", "Shadow Ink", "Simulation", "Sky", "Sleet", "Slime", "Slush", "Snowfall", "Snowy Night", "Solar", "Soul Scourge", "Spartan", "Spectral", "Speed", "Spellslinger", "Spiral", "Stained", "Starblaze", "Starfall", "Steel", "Stocking Fur", "Storm", "Street Camo", "Strobe", "Studded", "Studs", "Sunset", "Sunset Sparkle", "Supernova", "Surge", "Swirls", "TIX", "TV Error", "TV Static", "Tawny", "Teal", "Tealur", "Tempest", "Termination", "The Heights", "Thunderburst", "Tiger", "Titanium", "Toy", "Triplaser", "Trophy", "Tungsten", "Tusky", "Ugly Sweater", "Ultra Zed", "Ultrablue", "Universal", "Urban Camo", "Venom", "Vexed", "Vigor", "Vile", "Violet", "Virus", "Voltaic", "Waste", "Water", "Watermelon", "Wealth", "Webbed", "Well Done", "Werewolf Fur", "White", "Winter Solstice", "Winter Wrapping", "Wintergreen", "Wintry", "Woven", "Yang", "Yellow", "Yin", "Zombie"}
...
local function SaveConfig()
    local ok = pcall(function()
        local data = {}
        for weapon, info in pairs(_G.rivalsEquipped) do
            data[weapon] = { 
                Skin = info.Skin or "Default", 
                Wrap = info.Wrap or "None",
                Charm = info.Charm or "None",
                Finisher = info.Finisher or "None"
            }
        end
        -- Save global finisher separately
        data.__global = {
            Finisher = _G.rivalsGlobalFinisher or "None"
        }
...
local function LoadConfig()
    ...
    if ok and result then
        for weapon, info in pairs(result) do
            if weapon ~= "__global" and _G.rivalsEquipped[weapon] then
                _G.rivalsEquipped[weapon].Skin = info.Skin or "Default"
                _G.rivalsEquipped[weapon].Wrap  = info.Wrap  or "None"
                _G.rivalsEquipped[weapon].Charm  = info.Charm  or "None"
                _G.rivalsEquipped[weapon].Finisher  = info.Finisher  or "None"
            end
        end
        -- Load global finisher
        if result.__global then
            _G.rivalsGlobalFinisher = result.__global.Finisher or "None"
        end
        return true
    end
    return false
...
-- GLOBAL STATE
-- ============================================================
_G.rivalsEquipped = _G.rivalsEquipped or {}
_G.rivalsGlobalFinisher = _G.rivalsGlobalFinisher or "None"  -- Finishers are global, not per-weapon
for weapon in pairs(SkinLists) do
    if not _G.rivalsEquipped[weapon] then
        _G.rivalsEquipped[weapon] = { 
            Skin = "Default", 
            Wrap = "None",
            Charm = "None",
            Finisher = "None"  -- Stored per-weapon in UI, but applied globally
        }
    end
end
LoadConfig()
...
local RarityColors = {
    ...
    Common       = Color3.fromRGB(123, 255,   0),
    Rare         = Color3.fromRGB(  0, 221, 255),
    Legendary    = Color3.fromRGB(255,   0,   0),
    Mythical     = Color3.fromRGB( 64,  68, 186),
    Unobtainable = Color3.fromRGB( 30,  30,  30),
}
local FALLBACK_COLOR = Color3.fromRGB(80, 80, 100)

-- ============================================================
-- MODULE LOCALS  (populated after task.spawn loads)
-- ============================================================
local CosmeticLibrary, _ItemLibrary, ClientViewModel, ReplicatedClass

local function GetSkinData(skinName)
    if not CosmeticLibrary then return nil end
    local entry = CosmeticLibrary.Cosmetics and CosmeticLibrary.Cosmetics[skinName]
    if not entry then return nil end
    return {
        Image  = entry.ImageHighResolution or entry.Image or "",
...
-- ============================================================
-- INSTANCE THEME  (matches main Instance menu palette)
-- ============================================================
local Theme = {
    ...
    Font       = Enum.Font.Code,
    FontUi     = Enum.Font.Code,
    Radius     = 0,
}

-- ============================================================
-- UI HELPERS  (Instance style)
-- ============================================================
local function mkF(parent, size, pos, color, radius)
    local f = Instance.new("Frame", parent)
    f.Size = size
    f.Position = pos
    f.BackgroundColor3 = color or Theme.Main
    f.BorderSizePixel = 0
    local r = radius
    if r == nil then r = Theme.Radius end
    if r and r > 0 then
...
local STATUS_H = 28
local LEFT_W   = 200
local HEADER_H = TITLE_H + TAB_H -- title + tab strip
...
-- Main window
local win = mkF(sg,
    UDim2.new(0, MAIN_W, 0, MAIN_H),
    UDim2.new(0.5, -MAIN_W/2, 0.5, -MAIN_H/2),
    Theme.Background, 0)
win.Active = true
win.Draggable = true
win.Visible = false
win.ClipsDescendants = true
mkStroke(win, Theme.Outline, 1)
local winAccent = Instance.new("Frame", win)
winAccent.Name = "AccentEdge"
winAccent.Size = UDim2.new(0, 2, 1, 0)
winAccent.Position = UDim2.new(0, 0, 0, 0)
winAccent.BackgroundColor3 = Theme.Accent
...
local tbar = mkF(win,
    ...
    Theme.Main, 0)
tbar.ZIndex = 15
mkStroke(tbar, Theme.Outline, 1)

local tLabel = mkLabel(tbar, {
    Size = UDim2.new(1, -120, 1, 0),
    Position = UDim2.new(0, 12, 0, 0),
    Text = "instance  |  skin changer",
    TextSize = 14,
    TextColor3 = Theme.FontColor,
    ZIndex = 16,
})
local tAccent = mkLabel(tbar, {
...
mkStroke(colBtn, Theme.Outline, 1)

local closeBtn = mkBtn(tbar, {
    Size = UDim2.new(0, 22, 0, 18),
...
mkStroke(closeBtn, Theme.Outline, 1)
closeBtn.MouseButton1Click:Connect(function()
    menuVisible = false
    win.Visible = false
end)

-- Tab bar under title — skins / wraps / charms / finishers / misc (all features kept)
-- NOTE: do NOT use UIListLayout + scale size (buttons collapse to 0 width on many clients)
...
tabBarBottom.Name = "TabBarBottomLine"
tabBarBottom.Size = UDim2.new(1, 0, 0, 1)
tabBarBottom.Position = UDim2.new(0, 0, 1, -1)
tabBarBottom.BackgroundColor3 = Theme.Outline
tabBarBottom.BorderSizePixel = 0
tabBarBottom.ZIndex = 17
tabBarBottom.Parent = tabBar

local TAB_NAMES = { "Skins", "Wraps", "Charms", "Finishers", "Misc" }
local TAB_COUNT = #TAB_NAMES
local tabs = {}
local tabContents = {}
local currentTab = "Skins"
-- menuVisible declared early for close button above
-- Forward ref: unlock button wires to runHooks after hooks are defined
local requestUnlockSkinChanger = nil

local function setTabSelected(name)
    for tName, tData in pairs(tabs) do
        local on = (tName == name)
        tData.btn.TextColor3 = on and Theme.FontColor or Theme.Muted
        tData.btn.BackgroundColor3 = on and Color3.fromRGB(28, 28, 28) or Color3.fromRGB(22, 22, 22)
        tData.indicator.Visible = on
        if tabContents[tName] then
            tabContents[tName].Visible = on
        end
    end
    currentTab = name
end
...
local function createTab(name, order)
    local display = string.lower(name)
    local idx = order - 1 -- 0-based for position

    -- Manual equal-width placement (reliable; list layout + scale often hides tabs)
    ...
    btn.Position = UDim2.new(idx / TAB_COUNT, 0, 0, 0)
    btn.Font = Theme.Font
    btn.Text = display
    btn.TextSize = 13
    ...
    content.Size = UDim2.new(1, -2, 1, -(HEADER_H + STATUS_H))
    content.Position = UDim2.new(0, 2, 0, HEADER_H)
...
local skinsTab = createTab("Skins", 1)
local wrapsTab = createTab("Wraps", 2)
local charmsTab = createTab("Charms", 3)
local finishersTab = createTab("Finishers", 4)
local miscTab = createTab("Misc", 5)

-- Initialize first tab (skins + wraps + charms + finishers + misc all still exist)
setTabSelected("Skins")

-- Body (everything below title bar) - Now inside Skins Tab
local body = Instance.new("Frame", skinsTab)
body.Name = "Body"
...
local leftPanel = mkF(body,
    UDim2.new(0, LEFT_W, 1, -10),
    UDim2.new(0, 6, 0, 6),
    Theme.Panel, 0)
mkStroke(leftPanel, Theme.Outline, 1)
...
searchBox.Text = ""
searchBox.TextColor3 = Theme.FontColor
searchBox.Font = Theme.Font
searchBox.TextSize = 13
searchBox.ClearTextOnFocus = false
Instance.new("UICorner", searchBox).CornerRadius = UDim.new(0, 0)
mkStroke(searchBox, Theme.Outline, 1)
local searchPad = Instance.new("UIPadding", searchBox)
searchPad.PaddingLeft = UDim.new(0, 0)

-- Weapon scroll
local wScroll = Instance.new("ScrollingFrame", leftPanel)
wScroll.Size = UDim2.new(1, -8, 1, -50)
wScroll.Position = UDim2.new(0, 4, 0, 46)
wScroll.BackgroundTransparency = 1
wScroll.ScrollBarThickness = 4
wScroll.ScrollBarImageColor3 = Theme.Dim
...
local rightPanel = mkF(body,
    UDim2.new(1, -(LEFT_W + 18), 1, -10),
    UDim2.new(0, LEFT_W + 12, 0, 6),
    Theme.Panel, 0)
mkStroke(rightPanel, Theme.Outline, 1)

-- Weapon name label at top of right panel
local weaponLabel = Instance.new("TextLabel", rightPanel)
weaponLabel.Size = UDim2.new(1, -16, 0, 34)
weaponLabel.Position = UDim2.new(0, 8, 0, 6)
weaponLabel.BackgroundTransparency = 1
weaponLabel.Text = "Select a weapon"
weaponLabel.TextColor3 = Color3.fromRGB(180, 180, 190)
weaponLabel.Font = Theme.Font
weaponLabel.TextSize = 16
weaponLabel.TextXAlignment = Enum.TextXAlignment.Left

-- Skin scroll (grid)
local sScroll = Instance.new("ScrollingFrame", rightPanel)
sScroll.Size = UDim2.new(1, -12, 1, -50)
sScroll.Position = UDim2.new(0, 6, 0, 44)
sScroll.BackgroundTransparency = 1
sScroll.ScrollBarThickness = 4
sScroll.ScrollBarImageColor3 = Theme.Dim
sScroll.BorderSizePixel = 0
sScroll.CanvasSize = UDim2.new(0, 0, 0, 0)

local sGrid = Instance.new("UIGridLayout", sScroll)
sGrid.CellSize = UDim2.new(0, 140, 0, 172)
sGrid.CellPadding = UDim2.new(0, 12, 0, 12)
...
-- WRAPS TAB
-- ============================================================
local wrapsBody = Instance.new("Frame", wrapsTab)
wrapsBody.Name = "Body"
wrapsBody.Size = UDim2.new(1, 0, 1, 0)
wrapsBody.Position = UDim2.new(0, 0, 0, 0)
wrapsBody.BackgroundTransparency = 1

-- Left panel for wraps (weapon list)
local wrapsLeftPanel = mkF(wrapsBody,
    UDim2.new(0, LEFT_W, 1, -10),
    UDim2.new(0, 8, 0, 5),
    Theme.Panel, 8)
mkStroke(wrapsLeftPanel, Theme.Outline, 1)

local wrapsSearchBox = Instance.new("TextBox", wrapsLeftPanel)
wrapsSearchBox.Size = UDim2.new(1, -16, 0, 30)
wrapsSearchBox.Position = UDim2.new(0, 8, 0, 8)
wrapsSearchBox.BackgroundColor3 = Theme.Input
wrapsSearchBox.BorderSizePixel = 0
wrapsSearchBox.PlaceholderText = "Search weapon..."
wrapsSearchBox.PlaceholderColor3 = Theme.Dim
wrapsSearchBox.Text = ""
wrapsSearchBox.TextColor3 = Theme.FontColor
wrapsSearchBox.Font = Theme.Font
wrapsSearchBox.TextSize = 13
wrapsSearchBox.ClearTextOnFocus = false
Instance.new("UICorner", wrapsSearchBox).CornerRadius = UDim.new(0, 0)
mkStroke(wrapsSearchBox, Theme.Outline, 1)
local wrapsSearchPad = Instance.new("UIPadding", wrapsSearchBox)
wrapsSearchPad.PaddingLeft = UDim.new(0, 0)

local wrapsWScroll = Instance.new("ScrollingFrame", wrapsLeftPanel)
wrapsWScroll.Size = UDim2.new(1, -8, 1, -50)
wrapsWScroll.Position = UDim2.new(0, 4, 0, 46)
wrapsWScroll.BackgroundTransparency = 1
wrapsWScroll.ScrollBarThickness = 4
wrapsWScroll.ScrollBarImageColor3 = Theme.Dim
wrapsWScroll.BorderSizePixel = 0
wrapsWScroll.CanvasSize = UDim2.new(0, 0, 0, 0)

local wrapsWLayout = Instance.new("UIListLayout", wrapsWScroll)
wrapsWLayout.Padding = UDim.new(0, 4)
wrapsWLayout.SortOrder = Enum.SortOrder.Name
wrapsWLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center

local wrapsWPad = Instance.new("UIPadding", wrapsWScroll)
wrapsWPad.PaddingTop = UDim.new(0, 4)
wrapsWPad.PaddingBottom = UDim.new(0, 4)

-- Right panel for wraps (wrap grid)
local wrapsRightPanel = mkF(wrapsBody,
    UDim2.new(1, -(LEFT_W + 24), 1, -10),
    UDim2.new(0, LEFT_W + 16, 0, 5),
    Theme.Panel, 8)
mkStroke(wrapsRightPanel, Theme.Outline, 1)

local wrapsWeaponLabel = Instance.new("TextLabel", wrapsRightPanel)
wrapsWeaponLabel.Size = UDim2.new(1, -16, 0, 34)
wrapsWeaponLabel.Position = UDim2.new(0, 8, 0, 6)
wrapsWeaponLabel.BackgroundTransparency = 1
wrapsWeaponLabel.Text = "Select a weapon for wrap"
wrapsWeaponLabel.TextColor3 = Color3.fromRGB(180, 180, 190)
wrapsWeaponLabel.Font = Theme.Font
wrapsWeaponLabel.TextSize = 16
wrapsWeaponLabel.TextXAlignment = Enum.TextXAlignment.Left

local wrapsScroll = Instance.new("ScrollingFrame", wrapsRightPanel)
wrapsScroll.Size = UDim2.new(1, -12, 1, -50)
wrapsScroll.Position = UDim2.new(0, 6, 0, 44)
wrapsScroll.BackgroundTransparency = 1
wrapsScroll.ScrollBarThickness = 4
wrapsScroll.ScrollBarImageColor3 = Theme.Dim
wrapsScroll.BorderSizePixel = 0
wrapsScroll.CanvasSize = UDim2.new(0, 0, 0, 0)

local wrapsGrid = Instance.new("UIGridLayout", wrapsScroll)
wrapsGrid.CellSize = UDim2.new(0, 140, 0, 172)
wrapsGrid.CellPadding = UDim2.new(0, 12, 0, 12)
wrapsGrid.SortOrder = Enum.SortOrder.LayoutOrder

local wrapsPad = Instance.new("UIPadding", wrapsScroll)
wrapsPad.PaddingTop = UDim.new(0, 0)
wrapsPad.PaddingLeft = UDim.new(0, 0)
wrapsPad.PaddingBottom = UDim.new(0, 0)


-- ============================================================
-- CHARMS TAB (Per-Weapon Charm Selector)
-- ============================================================
local charmsBody = Instance.new("Frame", charmsTab)
charmsBody.Name = "Body"
charmsBody.Size = UDim2.new(1, 0, 1, 0)
charmsBody.Position = UDim2.new(0, 0, 0, 0)
charmsBody.BackgroundTransparency = 1

-- Left panel for charms (weapon list)
local charmsLeftPanel = mkF(charmsBody,
    UDim2.new(0, LEFT_W, 1, -10),
    UDim2.new(0, 8, 0, 5),
    Theme.Panel, 8)
mkStroke(charmsLeftPanel, Theme.Outline, 1)

local charmsWeaponSearchBox = Instance.new("TextBox", charmsLeftPanel)
charmsWeaponSearchBox.Size = UDim2.new(1, -16, 0, 30)
charmsWeaponSearchBox.Position = UDim2.new(0, 8, 0, 8)
charmsWeaponSearchBox.BackgroundColor3 = Theme.Input
charmsWeaponSearchBox.BorderSizePixel = 0
charmsWeaponSearchBox.PlaceholderText = "Search weapon..."
charmsWeaponSearchBox.PlaceholderColor3 = Theme.Dim
charmsWeaponSearchBox.Text = ""
charmsWeaponSearchBox.TextColor3 = Theme.FontColor
charmsWeaponSearchBox.Font = Theme.Font
charmsWeaponSearchBox.TextSize = 13
charmsWeaponSearchBox.ClearTextOnFocus = false
Instance.new("UICorner", charmsWeaponSearchBox).CornerRadius = UDim.new(0, 0)
mkStroke(charmsWeaponSearchBox, Theme.Outline, 1)
local charmsWeaponSearchPad = Instance.new("UIPadding", charmsWeaponSearchBox)
charmsWeaponSearchPad.PaddingLeft = UDim.new(0, 0)

local charmsWScroll = Instance.new("ScrollingFrame", charmsLeftPanel)
charmsWScroll.Size = UDim2.new(1, -8, 1, -50)
charmsWScroll.Position = UDim2.new(0, 4, 0, 46)
charmsWScroll.BackgroundTransparency = 1
charmsWScroll.ScrollBarThickness = 4
charmsWScroll.ScrollBarImageColor3 = Theme.Dim
charmsWScroll.BorderSizePixel = 0
charmsWScroll.CanvasSize = UDim2.new(0, 0, 0, 0)

local charmsWLayout = Instance.new("UIListLayout", charmsWScroll)
charmsWLayout.Padding = UDim.new(0, 4)
charmsWLayout.SortOrder = Enum.SortOrder.Name
charmsWLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center

local charmsWPad = Instance.new("UIPadding", charmsWScroll)
charmsWPad.PaddingTop = UDim.new(0, 4)
charmsWPad.PaddingBottom = UDim.new(0, 4)

-- Right panel for charms (charm grid)
local charmsRightPanel = mkF(charmsBody,
    UDim2.new(1, -(LEFT_W + 24), 1, -10),
    UDim2.new(0, LEFT_W + 16, 0, 5),
    Theme.Panel, 8)
mkStroke(charmsRightPanel, Theme.Outline, 1)

local charmsWeaponLabel = Instance.new("TextLabel", charmsRightPanel)
charmsWeaponLabel.Size = UDim2.new(1, -16, 0, 34)
charmsWeaponLabel.Position = UDim2.new(0, 8, 0, 6)
charmsWeaponLabel.BackgroundTransparency = 1
charmsWeaponLabel.Text = "Select a weapon for charm"
charmsWeaponLabel.TextColor3 = Color3.fromRGB(180, 180, 190)
charmsWeaponLabel.Font = Theme.Font
charmsWeaponLabel.TextSize = 16
charmsWeaponLabel.TextXAlignment = Enum.TextXAlignment.Left

local charmsSearchBox = Instance.new("TextBox", charmsRightPanel)
charmsSearchBox.Size = UDim2.new(1, -16, 0, 30)
charmsSearchBox.Position = UDim2.new(0, 8, 0, 48)
charmsSearchBox.BackgroundColor3 = Theme.Input
charmsSearchBox.BorderSizePixel = 0
charmsSearchBox.PlaceholderText = "Search charm..."
charmsSearchBox.PlaceholderColor3 = Theme.Dim
charmsSearchBox.Text = ""
charmsSearchBox.TextColor3 = Theme.FontColor
charmsSearchBox.Font = Theme.Font
charmsSearchBox.TextSize = 13
charmsSearchBox.ClearTextOnFocus = false
Instance.new("UICorner", charmsSearchBox).CornerRadius = UDim.new(0, 0)
mkStroke(charmsSearchBox, Theme.Outline, 1)
local charmsSearchPad = Instance.new("UIPadding", charmsSearchBox)
charmsSearchPad.PaddingLeft = UDim.new(0, 0)

local charmsScroll = Instance.new("ScrollingFrame", charmsRightPanel)
charmsScroll.Size = UDim2.new(1, -12, 1, -94)
charmsScroll.Position = UDim2.new(0, 6, 0, 88)
charmsScroll.BackgroundTransparency = 1
charmsScroll.ScrollBarThickness = 4
charmsScroll.ScrollBarImageColor3 = Theme.Dim
charmsScroll.BorderSizePixel = 0
charmsScroll.CanvasSize = UDim2.new(0, 0, 0, 0)

local charmsGrid = Instance.new("UIGridLayout", charmsScroll)
charmsGrid.CellSize = UDim2.new(0, 140, 0, 172)
charmsGrid.CellPadding = UDim2.new(0, 12, 0, 12)
charmsGrid.SortOrder = Enum.SortOrder.LayoutOrder

local charmsPad = Instance.new("UIPadding", charmsScroll)
charmsPad.PaddingTop = UDim.new(0, 0)
charmsPad.PaddingLeft = UDim.new(0, 0)
charmsPad.PaddingBottom = UDim.new(0, 0)


-- ============================================================
-- FINISHERS TAB (Per-Weapon Finisher Selector)
-- ============================================================
local finishersBody = Instance.new("Frame", finishersTab)
finishersBody.Name = "Body"
finishersBody.Size = UDim2.new(1, 0, 1, 0)
finishersBody.Position = UDim2.new(0, 0, 0, 0)
finishersBody.BackgroundTransparency = 1

-- Left panel for finishers (weapon list)
local finishersLeftPanel = mkF(finishersBody,
    UDim2.new(0, LEFT_W, 1, -10),
    UDim2.new(0, 8, 0, 5),
    Theme.Panel, 8)
mkStroke(finishersLeftPanel, Theme.Outline, 1)

local finishersWeaponSearchBox = Instance.new("TextBox", finishersLeftPanel)
finishersWeaponSearchBox.Size = UDim2.new(1, -16, 0, 30)
finishersWeaponSearchBox.Position = UDim2.new(0, 8, 0, 8)
finishersWeaponSearchBox.BackgroundColor3 = Theme.Input
finishersWeaponSearchBox.BorderSizePixel = 0
finishersWeaponSearchBox.PlaceholderText = "Search weapon..."
finishersWeaponSearchBox.PlaceholderColor3 = Theme.Dim
finishersWeaponSearchBox.Text = ""
finishersWeaponSearchBox.TextColor3 = Theme.FontColor
finishersWeaponSearchBox.Font = Theme.Font
finishersWeaponSearchBox.TextSize = 13
finishersWeaponSearchBox.ClearTextOnFocus = false
Instance.new("UICorner", finishersWeaponSearchBox).CornerRadius = UDim.new(0, 0)
mkStroke(finishersWeaponSearchBox, Theme.Outline, 1)
local finishersWeaponSearchPad = Instance.new("UIPadding", finishersWeaponSearchBox)
finishersWeaponSearchPad.PaddingLeft = UDim.new(0, 0)

local finishersWScroll = Instance.new("ScrollingFrame", finishersLeftPanel)
finishersWScroll.Size = UDim2.new(1, -8, 1, -50)
finishersWScroll.Position = UDim2.new(0, 4, 0, 46)
finishersWScroll.BackgroundTransparency = 1
finishersWScroll.ScrollBarThickness = 4
finishersWScroll.ScrollBarImageColor3 = Theme.Dim
finishersWScroll.BorderSizePixel = 0
finishersWScroll.CanvasSize = UDim2.new(0, 0, 0, 0)

local finishersWLayout = Instance.new("UIListLayout", finishersWScroll)
finishersWLayout.Padding = UDim.new(0, 4)
finishersWLayout.SortOrder = Enum.SortOrder.Name
finishersWLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center

local finishersWPad = Instance.new("UIPadding", finishersWScroll)
finishersWPad.PaddingTop = UDim.new(0, 4)
finishersWPad.PaddingBottom = UDim.new(0, 4)

-- Right panel for finishers (finisher grid)
local finishersRightPanel = mkF(finishersBody,
    UDim2.new(1, -(LEFT_W + 24), 1, -10),
    UDim2.new(0, LEFT_W + 16, 0, 5),
    Theme.Panel, 8)
mkStroke(finishersRightPanel, Theme.Outline, 1)

local finishersWeaponLabel = Instance.new("TextLabel", finishersRightPanel)
finishersWeaponLabel.Size = UDim2.new(1, -16, 0, 34)
finishersWeaponLabel.Position = UDim2.new(0, 8, 0, 6)
finishersWeaponLabel.BackgroundTransparency = 1
finishersWeaponLabel.Text = "Select a weapon for finisher (Note: finishers apply globally in Rivals)"
finishersWeaponLabel.TextColor3 = Color3.fromRGB(180, 180, 190)
finishersWeaponLabel.Font = Theme.Font
finishersWeaponLabel.TextSize = 13
finishersWeaponLabel.TextXAlignment = Enum.TextXAlignment.Left
finishersWeaponLabel.TextWrapped = true

local finishersSearchBox = Instance.new("TextBox", finishersRightPanel)
finishersSearchBox.Size = UDim2.new(1, -16, 0, 30)
finishersSearchBox.Position = UDim2.new(0, 8, 0, 48)
finishersSearchBox.BackgroundColor3 = Theme.Input
finishersSearchBox.BorderSizePixel = 0
finishersSearchBox.PlaceholderText = "Search finisher..."
finishersSearchBox.PlaceholderColor3 = Theme.Dim
finishersSearchBox.Text = ""
finishersSearchBox.TextColor3 = Theme.FontColor
finishersSearchBox.Font = Theme.Font
finishersSearchBox.TextSize = 13
finishersSearchBox.ClearTextOnFocus = false
Instance.new("UICorner", finishersSearchBox).CornerRadius = UDim.new(0, 0)
mkStroke(finishersSearchBox, Theme.Outline, 1)
local finishersSearchPad = Instance.new("UIPadding", finishersSearchBox)
finishersSearchPad.PaddingLeft = UDim.new(0, 0)

local finishersScroll = Instance.new("ScrollingFrame", finishersRightPanel)
finishersScroll.Size = UDim2.new(1, -12, 1, -94)
finishersScroll.Position = UDim2.new(0, 6, 0, 88)
finishersScroll.BackgroundTransparency = 1
finishersScroll.ScrollBarThickness = 4
finishersScroll.ScrollBarImageColor3 = Theme.Dim
finishersScroll.BorderSizePixel = 0
finishersScroll.CanvasSize = UDim2.new(0, 0, 0, 0)

local finishersGrid = Instance.new("UIGridLayout", finishersScroll)
finishersGrid.CellSize = UDim2.new(0, 140, 0, 172)
finishersGrid.CellPadding = UDim2.new(0, 12, 0, 12)
finishersGrid.SortOrder = Enum.SortOrder.LayoutOrder

local finishersPad = Instance.new("UIPadding", finishersScroll)
finishersPad.PaddingTop = UDim.new(0, 0)
finishersPad.PaddingLeft = UDim.new(0, 0)
finishersPad.PaddingBottom = UDim.new(0, 0)

-- ============================================================
-- MISC TAB  (unlock skin changer + helpers)
-- ============================================================
local miscBody = Instance.new("Frame", miscTab)
miscBody.Name = "Body"
miscBody.Size = UDim2.new(1, 0, 1, 0)
miscBody.Position = UDim2.new(0, 0, 0, 0)
miscBody.BackgroundTransparency = 1

local miscPanel = mkF(miscBody,
    UDim2.new(1, -24, 1, -24),
    UDim2.new(0, 12, 0, 12),
    Theme.Panel, 0)
mkStroke(miscPanel, Theme.Outline, 1)

local miscTitle = Instance.new("TextLabel", miscPanel)
miscTitle.Size = UDim2.new(1, -24, 0, 28)
miscTitle.Position = UDim2.new(0, 12, 0, 12)
miscTitle.BackgroundTransparency = 1
miscTitle.Text = "misc"
miscTitle.TextColor3 = Theme.FontColor
miscTitle.Font = Theme.Font
miscTitle.TextSize = 14
miscTitle.TextXAlignment = Enum.TextXAlignment.Left

local miscDesc = Instance.new("TextLabel", miscPanel)
miscDesc.Size = UDim2.new(1, -24, 0, 52)
miscDesc.Position = UDim2.new(0, 12, 0, 44)
miscDesc.BackgroundTransparency = 1
miscDesc.Text = "Press Unlock Skin Changer to apply ClientViewModel hooks.\nSkins, wraps, charms, and finishers only show in-game after unlock."
miscDesc.TextColor3 = Theme.Dim
miscDesc.Font = Theme.Font
miscDesc.TextSize = 12
miscDesc.TextWrapped = true
miscDesc.TextXAlignment = Enum.TextXAlignment.Left
miscDesc.TextYAlignment = Enum.TextYAlignment.Top

local unlockStatus = Instance.new("TextLabel", miscPanel)
unlockStatus.Size = UDim2.new(1, -24, 0, 20)
unlockStatus.Position = UDim2.new(0, 12, 0, 104)
unlockStatus.BackgroundTransparency = 1
unlockStatus.Text = (_G.rivalsSCHooked and "status: unlocked") or (_G.rivalsSkinChangerUnlocked and "status: unlocking…") or "status: locked"
unlockStatus.TextColor3 = _G.rivalsSCHooked and Color3.fromRGB(80, 220, 120) or Theme.Muted
unlockStatus.Font = Theme.Font
unlockStatus.TextSize = 12
unlockStatus.TextXAlignment = Enum.TextXAlignment.Left

local unlockBtn = Instance.new("TextButton", miscPanel)
unlockBtn.Name = "UnlockSkinChanger"
unlockBtn.Size = UDim2.new(0, 220, 0, 36)
unlockBtn.Position = UDim2.new(0, 12, 0, 136)
unlockBtn.BackgroundColor3 = Color3.fromRGB(40, 110, 40)
unlockBtn.BorderSizePixel = 0
unlockBtn.AutoButtonColor = false
unlockBtn.Text = "Unlock Skin Changer"
unlockBtn.TextColor3 = Color3.new(1, 1, 1)
unlockBtn.Font = Theme.Font
unlockBtn.TextSize = 13
Instance.new("UICorner", unlockBtn).CornerRadius = UDim.new(0, 4)
mkStroke(unlockBtn, Theme.Outline, 1)

local unlockHint = Instance.new("TextLabel", miscPanel)
unlockHint.Size = UDim2.new(1, -24, 0, 36)
unlockHint.Position = UDim2.new(0, 12, 0, 184)
unlockHint.BackgroundTransparency = 1
unlockHint.Text = "Tip: if status stays locked, join a match/lobby and press again (or Re-apply Hooks)."
unlockHint.TextColor3 = Theme.Dim
unlockHint.Font = Theme.Font
unlockHint.TextSize = 11
unlockHint.TextWrapped = true
unlockHint.TextXAlignment = Enum.TextXAlignment.Left
unlockHint.TextYAlignment = Enum.TextYAlignment.Top

local function refreshUnlockStatus()
    local on = _G.rivalsSCHooked == true
    local want = _G.rivalsSkinChangerUnlocked == true
    if on then
        unlockStatus.Text = "status: unlocked"
        unlockStatus.TextColor3 = Color3.fromRGB(80, 220, 120)
        unlockBtn.Text = "Re-apply Hooks"
        unlockBtn.BackgroundColor3 = Color3.fromRGB(40, 80, 160)
    elseif want then
        unlockStatus.Text = "status: locked (waiting for modules)"
        unlockStatus.TextColor3 = Color3.fromRGB(220, 160, 80)
        unlockBtn.Text = "Retry Unlock"
        unlockBtn.BackgroundColor3 = Color3.fromRGB(40, 110, 40)
    else
        unlockStatus.Text = "status: locked"
        unlockStatus.TextColor3 = Theme.Muted
        unlockBtn.Text = "Unlock Skin Changer"
        unlockBtn.BackgroundColor3 = Color3.fromRGB(40, 110, 40)
    end
end
...
unlockBtn.MouseEnter:Connect(function()
    unlockBtn.BackgroundColor3 = (_G.rivalsSCHooked and Color3.fromRGB(50, 100, 190) or Color3.fromRGB(50, 140, 50))
end)
unlockBtn.MouseLeave:Connect(function()
    refreshUnlockStatus()
end)
unlockBtn.MouseButton1Click:Connect(function()
    if requestUnlockSkinChanger then
        requestUnlockSkinChanger()
    else
        unlockStatus.Text = "status: hooks not ready yet"
        unlockStatus.TextColor3 = Color3.fromRGB(220, 160, 80)
    end
    task.defer(refreshUnlockStatus)
end)

-- Status bar at very bottom
local statusBar = mkF(win,
    UDim2.new(1, 0, 0, 28),
    UDim2.new(0, 0, 1, -28),
    Theme.Background, 0)
local statusBarTop = Instance.new("Frame", statusBar)
statusBarTop.Size = UDim2.new(1, 0, 0, 8)
statusBarTop.Position = UDim2.new(0, 0, 0, 0)
statusBarTop.BackgroundColor3 = Theme.Background
statusBarTop.BorderSizePixel = 0
...
-- FlashStatus: guarded so delayed callback can't crash after GUI is destroyed
...
-- Save / Load buttons in status bar
local function mkBarBtn(text, xOff, col)
    local btn = Instance.new("TextButton", statusBar)
...
local saveBtn = mkBarBtn("Save Config", -196, Color3.fromRGB(40, 110, 40))
local loadBtn = mkBarBtn("Load Config", -100, Color3.fromRGB(40, 80, 160))

saveBtn.MouseButton1Click:Connect(function()
    if SaveConfig() then FlashStatus("Config saved.", Color3.fromRGB(80, 220, 120))
    else FlashStatus("Save failed.", Color3.fromRGB(220, 80, 80)) end
end)
loadBtn.MouseButton1Click:Connect(function()
    if LoadConfig() then FlashStatus("Config loaded.", Theme.Accent)
...
-- SKIN CARD BUILDER
-- ============================================================
local _currentWeapon = nil

local function ClearSkinGrid()
...
local function BuildSkinGrid(weapon)
    ClearSkinGrid()
    _currentWeapon = weapon
    weaponLabel.Text = weapon

    local skins = SkinLists[weapon]
    if not skins then return end

    for i, skin in ipairs(skins) do
        local isEquipped = _G.rivalsEquipped[weapon] and _G.rivalsEquipped[weapon].Skin == skin
        local skinData   = GetSkinData(skin)
        local rarityCol  = (skinData and RarityColors[skinData.Rarity]) or FALLBACK_COLOR

        local card = Instance.new("Frame", sScroll)
        card.BackgroundColor3 = Color3.fromRGB(26, 26, 30)
        card.BorderSizePixel = 0
        card.LayoutOrder = i
        Instance.new("UICorner", card).CornerRadius = UDim.new(0, 0)

        local stroke = mkStroke(card, isEquipped and Color3.fromRGB(80, 220, 80) or rarityCol,
            isEquipped and 3 or 2)
        stroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border

        local imgBtn = Instance.new("ImageButton", card)
        imgBtn.Size = UDim2.new(1, 0, 1, 0)
        imgBtn.BackgroundTransparency = 1
        imgBtn.Image = (skinData and skinData.Image ~= "") and skinData.Image or "rbxassetid://0"
        imgBtn.ScaleType = Enum.ScaleType.Fit
        imgBtn.BorderSizePixel = 0

        local grad = Instance.new("Frame", imgBtn)
        grad.Size = UDim2.new(1, 0, 0, 56)
        grad.Position = UDim2.new(0, 0, 1, -56)
        grad.BackgroundColor3 = Color3.new(0, 0, 0)
        grad.BackgroundTransparency = 0.2
        grad.BorderSizePixel = 0
        Instance.new("UICorner", grad).CornerRadius = UDim.new(0, 0)

        if skinData and skinData.Rarity then
            local badge = Instance.new("TextLabel", imgBtn)
            badge.Size = UDim2.new(0, 68, 0, 16)
            badge.Position = UDim2.new(1, -72, 0, 4)
            badge.BackgroundColor3 = rarityCol
            badge.BackgroundTransparency = 0.1
            badge.Text = skinData.Rarity
        ...
        local nameLbl = Instance.new("TextLabel", imgBtn)
        nameLbl.Size = UDim2.new(1, -6, 0, 36)
        nameLbl.Position = UDim2.new(0, 3, 1, -38)
        ...
        imgBtn.MouseButton1Click:Connect(function()
            for _, c in ipairs(sScroll:GetChildren()) do
                if c:IsA("Frame") then
                    ...
                    local ib = c:FindFirstChildOfClass("ImageButton")
                    if s and ib then
                        local nl = ib:FindFirstChildWhichIsA("TextLabel")
                        local sName = nl and nl.Text or ""
                        local sd = GetSkinData(sName)
                        local rc = (sd and RarityColors[sd.Rarity]) or FALLBACK_COLOR
                        s.Color = rc
                        s.Thickness = 2
                    end
                end
            ...
            stroke.Thickness = 3
            _G.rivalsEquipped[weapon].Skin = skin
            weaponLabel.Text = weapon .. "  \xe2\x80\x94  " .. skin
            FlashStatus("Equipped: " .. weapon .. " / " .. skin, Color3.fromRGB(80, 220, 120))
        end)

        imgBtn.MouseEnter:Connect(function()
            local equipped = _G.rivalsEquipped[weapon] and _G.rivalsEquipped[weapon].Skin == skin
            if not equipped then stroke.Color = Color3.fromRGB(180, 180, 255) end
        end)
        imgBtn.MouseLeave:Connect(function()
            local equipped = _G.rivalsEquipped[weapon] and _G.rivalsEquipped[weapon].Skin == skin
            stroke.Color = equipped and Color3.fromRGB(80, 220, 80) or rarityCol
            stroke.Thickness = equipped and 3 or 2
        end)
    end

    sScroll.CanvasSize = UDim2.new(0, 0, 0, sGrid.AbsoluteContentSize.Y + 16)
end
...
-- WRAP GRID BUILDER
-- ============================================================
local _currentWrapWeapon = nil
local _wrapsList = nil

local function ClearWrapGrid()
    for _, c in ipairs(wrapsScroll:GetChildren()) do
        if not c:IsA("UIGridLayout") and not c:IsA("UIPadding") then
            c:Destroy()
        end
    end
...
local function BuildWrapGrid(weapon)
    ClearWrapGrid()
    _currentWrapWeapon = weapon
    wrapsWeaponLabel.Text = weapon .. " - Wrap"

    if not _wrapsList then return end

    -- Add "None" option
    local noneEquipped = _G.rivalsEquipped[weapon] and _G.rivalsEquipped[weapon].Wrap == "None"
    local noneCard = Instance.new("Frame", wrapsScroll)
    noneCard.BackgroundColor3 = Color3.fromRGB(26, 26, 30)
    noneCard.BorderSizePixel = 0
    noneCard.LayoutOrder = 0
    Instance.new("UICorner", noneCard).CornerRadius = UDim.new(0, 0)
    
    local noneStroke = mkStroke(noneCard, noneEquipped and Color3.fromRGB(80, 220, 80) or Theme.Dim,
        noneEquipped and 3 or 2)
    noneStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    
    local noneBtn = Instance.new("TextButton", noneCard)
    noneBtn.Size = UDim2.new(1, 0, 1, 0)
    noneBtn.BackgroundTransparency = 1
    noneBtn.Text = "None"
    noneBtn.TextColor3 = Color3.new(1, 1, 1)
    noneBtn.Font = Theme.Font
    noneBtn.TextSize = 16
    ...
    noneBtn.MouseButton1Click:Connect(function()
        for _, c in ipairs(wrapsScroll:GetChildren()) do
            if c:IsA("Frame") then
                local s = c:FindFirstChildOfClass("UIStroke")
                if s then s.Color = Theme.Dim; s.Thickness = 2 end
            end
        end
        noneStroke.Color = Color3.fromRGB(80, 220, 80)
        noneStroke.Thickness = 3
        _G.rivalsEquipped[weapon].Wrap = "None"
        FlashStatus("Removed wrap from: " .. weapon, Color3.fromRGB(80, 220, 120))
    end)

    for i, wrapName in ipairs(_wrapsList) do
        local isEquipped = _G.rivalsEquipped[weapon] and _G.rivalsEquipped[weapon].Wrap == wrapName
        local wrapData = GetSkinData(wrapName)
        local rarityCol = (wrapData and RarityColors[wrapData.Rarity]) or FALLBACK_COLOR

        local card = Instance.new("Frame", wrapsScroll)
        card.BackgroundColor3 = Color3.fromRGB(26, 26, 30)
        card.BorderSizePixel = 0
        card.LayoutOrder = i
        Instance.new("UICorner", card).CornerRadius = UDim.new(0, 0)
        ...
        imgBtn.Size = UDim2.new(1, 0, 1, 0)
        imgBtn.BackgroundTransparency = 1
        imgBtn.Image = (wrapData and wrapData.Image ~= "") and wrapData.Image or "rbxassetid://0"
        imgBtn.ScaleType = Enum.ScaleType.Fit
        imgBtn.BorderSizePixel = 0
        ...
        if wrapData and wrapData.Rarity then
            local badge = Instance.new("TextLabel", imgBtn)
            badge.Size = UDim2.new(0, 68, 0, 16)
            badge.Position = UDim2.new(1, -72, 0, 4)
            badge.BackgroundColor3 = rarityCol
            badge.BackgroundTransparency = 0.1
            badge.Text = wrapData.Rarity
            badge.TextColor3 = Color3.new(1, 1, 1)
            badge.Font = Theme.Font
            badge.TextSize = 9
            badge.BorderSizePixel = 0
        ...
        imgBtn.MouseButton1Click:Connect(function()
            for _, c in ipairs(wrapsScroll:GetChildren()) do
                if c:IsA("Frame") then
                    local s = c:FindFirstChildOfClass("UIStroke")
                    local ib = c:FindFirstChildOfClass("ImageButton") or c:FindFirstChildOfClass("TextButton")
                    if s and ib then
                        local nl = ib:FindFirstChildWhichIsA("TextLabel")
                        local name = nl and nl.Text or (ib:IsA("TextButton") and ib.Text) or ""
                        if name == "None" then
                            s.Color = Theme.Dim
                            s.Thickness = 2
                        else
                            local sd = GetSkinData(name)
                            local rc = (sd and RarityColors[sd.Rarity]) or FALLBACK_COLOR
                            s.Color = rc
                            s.Thickness = 2
                        end
            ...
            stroke.Color = Color3.fromRGB(80, 220, 80)
            stroke.Thickness = 3
            _G.rivalsEquipped[weapon].Wrap = wrapName
            FlashStatus("Equipped wrap: " .. weapon .. " / " .. wrapName, Color3.fromRGB(80, 220, 120))
        end)

        imgBtn.MouseEnter:Connect(function()
            local equipped = _G.rivalsEquipped[weapon] and _G.rivalsEquipped[weapon].Wrap == wrapName
            if not equipped then stroke.Color = Color3.fromRGB(180, 180, 255) end
        end)
        imgBtn.MouseLeave:Connect(function()
            local equipped = _G.rivalsEquipped[weapon] and _G.rivalsEquipped[weapon].Wrap == wrapName
    ...
    wrapsScroll.CanvasSize = UDim2.new(0, 0, 0, wrapsGrid.AbsoluteContentSize.Y + 16)
end


-- ============================================================
-- CHARM GRID BUILDER (Per-Weapon)
-- ============================================================
local _charmsList = nil
local _currentCharmWeapon = nil

local function ClearCharmGrid()
    for _, c in ipairs(charmsScroll:GetChildren()) do
        if not c:IsA("UIGridLayout") and not c:IsA("UIPadding") then
            c:Destroy()
        end
    end
...
local function BuildCharmGrid(weapon)
    ClearCharmGrid()
    _currentCharmWeapon = weapon
    charmsWeaponLabel.Text = weapon .. " - Charm"

    if not _charmsList then return end

    -- Add "None" option
    local noneEquipped = _G.rivalsEquipped[weapon] and _G.rivalsEquipped[weapon].Charm == "None"
    local noneCard = Instance.new("Frame", charmsScroll)
    noneCard.BackgroundColor3 = Color3.fromRGB(26, 26, 30)
    noneCard.BorderSizePixel = 0
    noneCard.LayoutOrder = 0
    Instance.new("UICorner", noneCard).CornerRadius = UDim.new(0, 0)
    ...
    noneBtn.MouseButton1Click:Connect(function()
        ...
        noneStroke.Color = Color3.fromRGB(80, 220, 80)
        noneStroke.Thickness = 3
        _G.rivalsEquipped[weapon].Charm = "None"
        FlashStatus("Removed charm from: " .. weapon, Color3.fromRGB(80, 220, 120))
    end)

    for i, charmName in ipairs(_charmsList) do
        local isEquipped = _G.rivalsEquipped[weapon] and _G.rivalsEquipped[weapon].Charm == charmName
        local charmData = GetSkinData(charmName)
        local rarityCol = (charmData and RarityColors[charmData.Rarity]) or FALLBACK_COLOR

        local card = Instance.new("Frame", charmsScroll)
        card.BackgroundColor3 = Color3.fromRGB(26, 26, 30)
        card.BorderSizePixel = 0
        card.LayoutOrder = i
        Instance.new("UICorner", card).CornerRadius = UDim.new(0, 0)
        ...
        imgBtn.Size = UDim2.new(1, 0, 1, 0)
        imgBtn.BackgroundTransparency = 1
        imgBtn.Image = (charmData and charmData.Image ~= "") and charmData.Image or "rbxassetid://0"
        imgBtn.ScaleType = Enum.ScaleType.Fit
        imgBtn.BorderSizePixel = 0
        ...
        if charmData and charmData.Rarity then
            local badge = Instance.new("TextLabel", imgBtn)
            badge.Size = UDim2.new(0, 68, 0, 16)
            badge.Position = UDim2.new(1, -72, 0, 4)
            badge.BackgroundColor3 = rarityCol
            badge.BackgroundTransparency = 0.1
            badge.Text = charmData.Rarity
            badge.TextColor3 = Color3.new(1, 1, 1)
            badge.Font = Theme.Font
            badge.TextSize = 9
            badge.BorderSizePixel = 0
        ...
        imgBtn.MouseButton1Click:Connect(function()
            ...
            stroke.Color = Color3.fromRGB(80, 220, 80)
            stroke.Thickness = 3
            _G.rivalsEquipped[weapon].Charm = charmName
            FlashStatus("Equipped charm: " .. weapon .. " / " .. charmName, Color3.fromRGB(80, 220, 120))
        end)

        imgBtn.MouseEnter:Connect(function()
            local equipped = _G.rivalsEquipped[weapon] and _G.rivalsEquipped[weapon].Charm == charmName
            if not equipped then stroke.Color = Color3.fromRGB(180, 180, 255) end
        end)
        imgBtn.MouseLeave:Connect(function()
            local equipped = _G.rivalsEquipped[weapon] and _G.rivalsEquipped[weapon].Charm == charmName
    ...
    charmsScroll.CanvasSize = UDim2.new(0, 0, 0, charmsGrid.AbsoluteContentSize.Y + 16)
end


-- ============================================================
-- FINISHER GRID BUILDER (Per-Weapon)
-- ============================================================
local _finishersList = nil
local _currentFinisherWeapon = nil

local function ClearFinisherGrid()
    for _, c in ipairs(finishersScroll:GetChildren()) do
        if not c:IsA("UIGridLayout") and not c:IsA("UIPadding") then
            c:Destroy()
        end
    end
...
local function BuildFinisherGrid(weapon)
    ClearFinisherGrid()
    _currentFinisherWeapon = weapon
    finishersWeaponLabel.Text = weapon .. " - Finisher"
    
    if not _finishersList then return end

    -- Add "None" option
    local noneEquipped = _G.rivalsEquipped[weapon] and _G.rivalsEquipped[weapon].Finisher == "None"
    local noneCard = Instance.new("Frame", finishersScroll)
    noneCard.BackgroundColor3 = Color3.fromRGB(26, 26, 30)
    noneCard.BorderSizePixel = 0
    noneCard.LayoutOrder = 0
    Instance.new("UICorner", noneCard).CornerRadius = UDim.new(0, 0)
    ...
    noneBtn.MouseButton1Click:Connect(function()
        ...
        noneStroke.Color = Color3.fromRGB(80, 220, 80)
        noneStroke.Thickness = 3
        _G.rivalsEquipped[weapon].Finisher = "None"
        _G.rivalsGlobalFinisher = "None"  -- Clear global finisher
        
        -- Remove finisher via remote (equip "None")
        pcall(function()
            local RS = game:GetService("ReplicatedStorage")
            local equipRemote = RS:FindFirstChild("Remotes")
            if equipRemote then
                equipRemote = equipRemote:FindFirstChild("Data")
                if equipRemote then
                    equipRemote = equipRemote:FindFirstChild("EquipCosmetic")
                    if equipRemote then
                        equipRemote:FireServer("None", "Finisher")
                    end
                end
            end
        end)
        
        FlashStatus("Removed finisher", Color3.fromRGB(80, 220, 120))
    end)

    for i, finisherName in ipairs(_finishersList) do
        local isEquipped = _G.rivalsEquipped[weapon] and _G.rivalsEquipped[weapon].Finisher == finisherName
        local finisherData = GetSkinData(finisherName)
        local rarityCol = (finisherData and RarityColors[finisherData.Rarity]) or FALLBACK_COLOR

        local card = Instance.new("Frame", finishersScroll)
        card.BackgroundColor3 = Color3.fromRGB(26, 26, 30)
        card.BorderSizePixel = 0
        card.LayoutOrder = i
        Instance.new("UICorner", card).CornerRadius = UDim.new(0, 0)
        ...
        imgBtn.Size = UDim2.new(1, 0, 1, 0)
        imgBtn.BackgroundTransparency = 1
        imgBtn.Image = (finisherData and finisherData.Image ~= "") and finisherData.Image or "rbxassetid://0"
        imgBtn.ScaleType = Enum.ScaleType.Fit
        imgBtn.BorderSizePixel = 0
        ...
        if finisherData and finisherData.Rarity then
            local badge = Instance.new("TextLabel", imgBtn)
            badge.Size = UDim2.new(0, 68, 0, 16)
            badge.Position = UDim2.new(1, -72, 0, 4)
            badge.BackgroundColor3 = rarityCol
            badge.BackgroundTransparency = 0.1
            badge.Text = finisherData.Rarity
            badge.TextColor3 = Color3.new(1, 1, 1)
            badge.Font = Theme.Font
            badge.TextSize = 9
            badge.BorderSizePixel = 0
        ...
        imgBtn.MouseButton1Click:Connect(function()
            ...
            stroke.Color = Color3.fromRGB(80, 220, 80)
            stroke.Thickness = 3
            _G.rivalsEquipped[weapon].Finisher = finisherName
            _G.rivalsGlobalFinisher = finisherName  -- Set global finisher (game uses this)
            
            -- Apply finisher via remote
            pcall(function()
                local RS = game:GetService("ReplicatedStorage")
                local equipRemote = RS:FindFirstChild("Remotes")
                if equipRemote then
                    ...
                    if equipRemote then
                        equipRemote = equipRemote:FindFirstChild("EquipCosmetic")
                        if equipRemote then
                            equipRemote:FireServer(finisherName, "Finisher")
                        end
                    end
                end
            end)
            
            FlashStatus("Equipped finisher: " .. finisherName .. " (applies globally)", Color3.fromRGB(80, 220, 120))
        end)

        imgBtn.MouseEnter:Connect(function()
            local equipped = _G.rivalsEquipped[weapon] and _G.rivalsEquipped[weapon].Finisher == finisherName
            if not equipped then stroke.Color = Color3.fromRGB(180, 180, 255) end
        end)
        imgBtn.MouseLeave:Connect(function()
            local equipped = _G.rivalsEquipped[weapon] and _G.rivalsEquipped[weapon].Finisher == finisherName
    ...
    finishersScroll.CanvasSize = UDim2.new(0, 0, 0, finishersGrid.AbsoluteContentSize.Y + 16)
end


-- ============================================================
-- WEAPON BUTTON BUILDER
-- ============================================================
local activeWeaponBtn = nil

local function MakeWeaponBtn(weapon)
    local btn = Instance.new("TextButton", wScroll)
    btn.Size = UDim2.new(1, -8, 0, 36)
    btn.BackgroundColor3 = Color3.fromRGB(28, 28, 32)
    btn.BorderSizePixel = 0
    btn.Text = ""
    Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 0)
    mkStroke(btn, Color3.fromRGB(38, 38, 44), 1)

    local dot = mkF(btn, UDim2.new(0, 7, 0, 7), UDim2.new(0, 9, 0.5, -3),
        Color3.fromRGB(55, 55, 65), 99)
    ...
    badge.Position = UDim2.new(1, -62, 0.5, -8)
    badge.BackgroundColor3 = Color3.fromRGB(180, 40, 40)
    badge.BorderSizePixel = 0
    badge.TextColor3 = Color3.new(1, 1, 1)
    badge.Font = Theme.Font
    badge.TextSize = 9
    ...
    local function RefreshBadge()
        local skin = _G.rivalsEquipped[weapon] and _G.rivalsEquipped[weapon].Skin or "Default"
        badge.Text = skin ~= "Default" and skin or ""
        badge.Visible = skin ~= "Default"
        dot.BackgroundColor3 = skin ~= "Default"
    ...
    btn.MouseButton1Click:Connect(function()
        if activeWeaponBtn and activeWeaponBtn ~= btn then
            activeWeaponBtn.BackgroundColor3 = Color3.fromRGB(28, 28, 32)
            local prevLbl = activeWeaponBtn:FindFirstChildWhichIsA("TextLabel")
            if prevLbl then prevLbl.TextColor3 = Color3.fromRGB(190, 190, 200) end
        end
        activeWeaponBtn = btn
        btn.BackgroundColor3 = Color3.fromRGB(38, 38, 50)
        lbl.TextColor3 = Color3.fromRGB(255, 255, 255)
        BuildSkinGrid(weapon)
        RefreshBadge()
    end)
...
-- Populate weapon list (for skins tab)
local weaponRefreshFns = {}
for weapon in pairs(SkinLists) do
    weaponRefreshFns[weapon] = MakeWeaponBtn(weapon)
end

-- Populate weapon list (for wraps tab)
local function MakeWrapWeaponBtn(weapon)
    local btn = Instance.new("TextButton", wrapsWScroll)
    btn.Size = UDim2.new(1, -8, 0, 36)
    btn.BackgroundColor3 = Color3.fromRGB(28, 28, 32)
    btn.BorderSizePixel = 0
    btn.Text = ""
    ...
    local function RefreshBadge()
        local wrap = _G.rivalsEquipped[weapon] and _G.rivalsEquipped[weapon].Wrap or "None"
        badge.Text = wrap ~= "None" and wrap or ""
        badge.Visible = wrap ~= "None"
        dot.BackgroundColor3 = wrap ~= "None"
            and Color3.fromRGB(80, 220, 80)
            or Color3.fromRGB(55, 55, 65)
    end
    RefreshBadge()
    ...
    btn.MouseButton1Click:Connect(function()
        for _, b in ipairs(wrapsWScroll:GetChildren()) do
            if b:IsA("TextButton") and b ~= btn then
                b.BackgroundColor3 = Color3.fromRGB(28, 28, 32)
                local prevLbl = b:FindFirstChildWhichIsA("TextLabel")
                if prevLbl then prevLbl.TextColor3 = Color3.fromRGB(190, 190, 200) end
            end
        end
        btn.BackgroundColor3 = Color3.fromRGB(38, 38, 50)
        lbl.TextColor3 = Color3.fromRGB(255, 255, 255)
        BuildWrapGrid(weapon)
        RefreshBadge()
    end)
...
local wrapWeaponRefreshFns = {}
for weapon in pairs(SkinLists) do
    wrapWeaponRefreshFns[weapon] = MakeWrapWeaponBtn(weapon)
end

-- Populate weapon list (for charms tab)
local function MakeCharmWeaponBtn(weapon)
    local btn = Instance.new("TextButton", charmsWScroll)
    btn.Size = UDim2.new(1, -8, 0, 36)
    btn.BackgroundColor3 = Color3.fromRGB(28, 28, 32)
    btn.BorderSizePixel = 0
    btn.Text = ""
    ...
    local function RefreshBadge()
        local charm = _G.rivalsEquipped[weapon] and _G.rivalsEquipped[weapon].Charm or "None"
        badge.Text = charm ~= "None" and charm or ""
        badge.Visible = charm ~= "None"
        dot.BackgroundColor3 = charm ~= "None"
            and Color3.fromRGB(80, 220, 80)
            or Color3.fromRGB(55, 55, 65)
    end
    RefreshBadge()
    ...
    btn.MouseButton1Click:Connect(function()
        for _, b in ipairs(charmsWScroll:GetChildren()) do
            if b:IsA("TextButton") and b ~= btn then
                b.BackgroundColor3 = Color3.fromRGB(28, 28, 32)
                local prevLbl = b:FindFirstChildWhichIsA("TextLabel")
                if prevLbl then prevLbl.TextColor3 = Color3.fromRGB(190, 190, 200) end
...
local charmWeaponRefreshFns = {}
for weapon in pairs(SkinLists) do
    charmWeaponRefreshFns[weapon] = MakeCharmWeaponBtn(weapon)
end

-- Populate weapon list (for finishers tab)
local function MakeFinisherWeaponBtn(weapon)
    local btn = Instance.new("TextButton", finishersWScroll)
    btn.Size = UDim2.new(1, -8, 0, 36)
    btn.BackgroundColor3 = Color3.fromRGB(28, 28, 32)
    btn.BorderSizePixel = 0
    btn.Text = ""
    ...
    local function RefreshBadge()
        local finisher = _G.rivalsEquipped[weapon] and _G.rivalsEquipped[weapon].Finisher or "None"
        badge.Text = finisher ~= "None" and finisher or ""
        badge.Visible = finisher ~= "None"
        dot.BackgroundColor3 = finisher ~= "None"
            and Color3.fromRGB(80, 220, 80)
            or Color3.fromRGB(55, 55, 65)
    end
    RefreshBadge()
    ...
    btn.MouseButton1Click:Connect(function()
        for _, b in ipairs(finishersWScroll:GetChildren()) do
            if b:IsA("TextButton") and b ~= btn then
                b.BackgroundColor3 = Color3.fromRGB(28, 28, 32)
                local prevLbl = b:FindFirstChildWhichIsA("TextLabel")
                if prevLbl then prevLbl.TextColor3 = Color3.fromRGB(190, 190, 200) end
...
local finisherWeaponRefreshFns = {}
for weapon in pairs(SkinLists) do
    finisherWeaponRefreshFns[weapon] = MakeFinisherWeaponBtn(weapon)
end

-- Canvas size updates
wScroll.CanvasSize = UDim2.new(0, 0, 0, wLayout.AbsoluteContentSize.Y + 8)
wrapsWScroll.CanvasSize = UDim2.new(0, 0, 0, wrapsWLayout.AbsoluteContentSize.Y + 8)
charmsWScroll.CanvasSize = UDim2.new(0, 0, 0, charmsWLayout.AbsoluteContentSize.Y + 8)
finishersWScroll.CanvasSize = UDim2.new(0, 0, 0, finishersWLayout.AbsoluteContentSize.Y + 8)

wLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    wScroll.CanvasSize = UDim2.new(0, 0, 0, wLayout.AbsoluteContentSize.Y + 8)
end)
wrapsWLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    wrapsWScroll.CanvasSize = UDim2.new(0, 0, 0, wrapsWLayout.AbsoluteContentSize.Y + 8)
end)
charmsWLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    charmsWScroll.CanvasSize = UDim2.new(0, 0, 0, charmsWLayout.AbsoluteContentSize.Y + 8)
end)
finishersWLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    finishersWScroll.CanvasSize = UDim2.new(0, 0, 0, finishersWLayout.AbsoluteContentSize.Y + 8)
end)
sGrid:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    sScroll.CanvasSize = UDim2.new(0, 0, 0, sGrid.AbsoluteContentSize.Y + 16)
end)
wrapsGrid:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    wrapsScroll.CanvasSize = UDim2.new(0, 0, 0, wrapsGrid.AbsoluteContentSize.Y + 16)
end)
charmsGrid:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    charmsScroll.CanvasSize = UDim2.new(0, 0, 0, charmsGrid.AbsoluteContentSize.Y + 16)
end)
finishersGrid:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    finishersScroll.CanvasSize = UDim2.new(0, 0, 0, finishersGrid.AbsoluteContentSize.Y + 16)
end)
...
searchBox:GetPropertyChangedSignal("Text"):Connect(function()
    ...
    for _, btn in ipairs(wScroll:GetChildren()) do
        if btn:IsA("TextButton") then
            local nameLbl = btn:FindFirstChildWhichIsA("TextLabel")
            local wName = nameLbl and nameLbl.Text:lower() or ""
            btn.Visible = query == "" or wName:find(query, 1, true) ~= nil
        end
    end
end)

wrapsSearchBox:GetPropertyChangedSignal("Text"):Connect(function()
    local query = wrapsSearchBox.Text:lower():match("^%s*(.-)%s*$")
    for _, btn in ipairs(wrapsWScroll:GetChildren()) do
        if btn:IsA("TextButton") then
            local nameLbl = btn:FindFirstChildWhichIsA("TextLabel")
            local wName = nameLbl and nameLbl.Text:lower() or ""
            btn.Visible = query == "" or wName:find(query, 1, true) ~= nil
...
charmsWeaponSearchBox:GetPropertyChangedSignal("Text"):Connect(function()
    local query = charmsWeaponSearchBox.Text:lower():match("^%s*(.-)%s*$")
    for _, btn in ipairs(charmsWScroll:GetChildren()) do
        if btn:IsA("TextButton") then
            local nameLbl = btn:FindFirstChildWhichIsA("TextLabel")
            local wName = nameLbl and nameLbl.Text:lower() or ""
            btn.Visible = query == "" or wName:find(query, 1, true) ~= nil
...
finishersWeaponSearchBox:GetPropertyChangedSignal("Text"):Connect(function()
    local query = finishersWeaponSearchBox.Text:lower():match("^%s*(.-)%s*$")
    for _, btn in ipairs(finishersWScroll:GetChildren()) do
        if btn:IsA("TextButton") then
            local nameLbl = btn:FindFirstChildWhichIsA("TextLabel")
            local wName = nameLbl and nameLbl.Text:lower() or ""
            btn.Visible = query == "" or wName:find(query, 1, true) ~= nil
...
charmsSearchBox:GetPropertyChangedSignal("Text"):Connect(function()
    local query = charmsSearchBox.Text:lower():match("^%s*(.-)%s*$")
    for _, card in ipairs(charmsScroll:GetChildren()) do
        if card:IsA("Frame") then
            local imgBtn = card:FindFirstChildOfClass("ImageButton") or card:FindFirstChildOfClass("TextButton")
            if imgBtn then
                local nameLbl = imgBtn:FindFirstChildWhichIsA("TextLabel")
                local name = nameLbl and nameLbl.Text or (imgBtn:IsA("TextButton") and imgBtn.Text) or ""
                card.Visible = query == "" or name:lower():find(query, 1, true) ~= nil
            end
        end
    end
end)

finishersSearchBox:GetPropertyChangedSignal("Text"):Connect(function()
    local query = finishersSearchBox.Text:lower():match("^%s*(.-)%s*$")
    for _, card in ipairs(finishersScroll:GetChildren()) do
        if card:IsA("Frame") then
            local imgBtn = card:FindFirstChildOfClass("ImageButton") or card:FindFirstChildOfClass("TextButton")
            if imgBtn then
                local nameLbl = imgBtn:FindFirstChildWhichIsA("TextLabel")
...
colBtn.MouseButton1Click:Connect(function()
    ...
    -- skins body is inside skins tab; keep consistent when expanding
    if body then
        body.Visible = true
    end
    colBtn.Text = collapsed and "+" or "-"
    win.Size = collapsed
        and UDim2.new(0, MAIN_W, 0, TITLE_H)
        or  UDim2.new(0, MAIN_W, 0, MAIN_H)
end)

-- menuVisible already declared
table.insert(_G.rivalsSkinConns,
...
-- ============================================================
-- MODULE LOADER (robust_require)
-- ============================================================
local MAX_GC_SCAN = 50000  -- cap GC scan to prevent timeout

local function robust_require(module, silent)
    ...
    local setid = rawget(genv, "setthreadidentity")
        or rawget(genv, "set_thread_identity")
        or (syn and rawget(syn, "set_thread_identity"))
    local getid = rawget(genv, "getthreadidentity")
        or rawget(genv, "get_thread_identity")
        or (syn and rawget(syn, "get_thread_identity"))

    local oldId
    pcall(function() if getid and setid then oldId = getid(); setid(2) end end)
    
    local ok, result = false, nil
    -- NEVER require ClientViewModel ourselves, it crashes the game if required too early.
    -- We will rely entirely on the getgc fallback to find it after the game loads it.
    if not mName:find("ClientViewModel") then
        -- Try standard require first
        ok, result = pcall(require, module)
        
        -- If standard require fails or returns something invalid, try global require
        if not ok or type(result) ~= "table" then
            local greq = rawget(genv, "require")
            if greq then 
                ok, result = pcall(greq, module) 
            end
        end
    end
    ...
    -- Check if the result is actually a valid module table and not an error string
    if ok and type(result) == "table" then return result end

    -- GC / registry scan fallback (only runs when require failed above)
    local gcfn  = rawget(genv, "getgc")
    local regfn = rawget(debug or {}, "getregistry")
    local scan_apis = {}
    if gcfn  then scan_apis[#scan_apis+1] = gcfn  end
    if regfn then scan_apis[#scan_apis+1] = regfn end
    for _, api in ipairs(scan_apis) do
        local s, objs = pcall(api, true)
        if s and type(objs) == "table" then
            local count = 0
            for _, v in ipairs(objs) do
                count = count + 1
                if count > MAX_GC_SCAN then break end  -- hard cap
                if type(v) == "table" then
                    local ok, _ = pcall(function()
                        if mName:find("CosmeticLibrary") and rawget(v, "Cosmetics") and type(rawget(v, "Equip")) == "function" then return true end
                        if mName:find("ItemLibrary") and rawget(v, "ViewModels") then return true end
                        if mName:find("ClientViewModel") and rawget(v, "new") and rawget(v, "GetWrap") then return true end
                        if mName:find("ReplicatedClass") and type(rawget(v, "ToEnum")) == "function" then return true end
                    end)
                    if ok and _ then return v end
                end
            end
        end
    end
    if not silent then
        warn("[Rivals SC] Failed to load: " .. mName)
    end
    return nil
end
...
-- ============================================================
-- SAFE POLL  (replaces WaitForChild — never yields a blocking thread)
-- WaitForChild yields the calling thread. If a teleport fires while that
-- thread is suspended, the executor tries to kill a stuck coroutine → crash.
-- safePoll spins with task.wait(0.1) instead, checking _G.rivalsShuttingDown
-- each tick so it exits immediately when a teleport starts.
-- ============================================================
local function safePoll(parent, name, timeout)
    local deadline = os.clock() + (timeout or 20)
    while os.clock() < deadline do
        if _G.rivalsShuttingDown then return nil end
...
-- ============================================================
-- HOOKS  (runHooks)
-- Uses safePoll instead of WaitForChild so no thread can get
-- stuck mid-yield during a teleport.
-- ============================================================
local function runHooks()
    if _G.rivalsSCHooked then return end
    _G.rivalsSCHooked = true

    -- Abort if we are already in a teleport transition
    if _G.rivalsShuttingDown then _G.rivalsSCHooked = false; return end

    local RS2 = game:GetService("ReplicatedStorage")

    -- Poll chain 1: shared modules
    local Modules = safePoll(RS2, "Modules", 20)
    if not Modules then
        FlashStatus("Module load failed (Modules).", Color3.fromRGB(220, 80, 80))
        _G.rivalsSCHooked = false
        return
    end
    if _G.rivalsShuttingDown then _G.rivalsSCHooked = false; return end

    local clLib = safePoll(Modules, "CosmeticLibrary", 20)
    local itLib = safePoll(Modules, "ItemLibrary",     20)
    local rcLib = safePoll(Modules, "ReplicatedClass", 20)
    if not clLib or not itLib or not rcLib then
        FlashStatus("Module load failed (children).", Color3.fromRGB(220, 80, 80))
        _G.rivalsSCHooked = false
        return
    end
    if _G.rivalsShuttingDown then _G.rivalsSCHooked = false; return end

    -- Set these as globals so they're accessible everywhere
    CosmeticLibrary = robust_require(clLib)
    _ItemLibrary    = robust_require(itLib)
    ReplicatedClass = robust_require(rcLib)
    
    -- Make them accessible globally (not just local to this function)
    _G.RivalsCosmeticLibrary = CosmeticLibrary
    _G.RivalsItemLibrary = _ItemLibrary
    _G.RivalsReplicatedClass = ReplicatedClass

    -- Poll chain 2: ClientViewModel
    if _G.rivalsShuttingDown then _G.rivalsSCHooked = false; return end
    local pScripts  = safePoll(player.PlayerScripts, "Modules", 15)
    local crClasses = pScripts and safePoll(pScripts, "ClientReplicatedClasses", 15)
    local cfPath    = crClasses and safePoll(crClasses, "ClientFighter", 15)
    local ciPath    = cfPath    and safePoll(cfPath,    "ClientItem",    15)
    local cvmModule = ciPath    and safePoll(ciPath,    "ClientViewModel", 15)
    if not cvmModule then
        FlashStatus("Module load failed (ClientViewModel).", Color3.fromRGB(220, 80, 80))
        _G.rivalsSCHooked = false
        return
    end
    if _G.rivalsShuttingDown then _G.rivalsSCHooked = false; return end
    
    -- Try requiring ClientViewModel with retries (it may have dependencies loading)
    -- Wrap in pcall to prevent breaking other game modules
    ClientViewModel = nil
    local cvmAttempts = 0
    while not ClientViewModel and cvmAttempts < 10 do
        if _G.rivalsShuttingDown then _G.rivalsSCHooked = false; return end
        local success, result = pcall(function()
            return require(cvmModule)
        end)
        if success and type(result) == "table" and result.new then
            ClientViewModel = result
            break
        end
        cvmAttempts = cvmAttempts + 1
        task.wait(1)
    end
    
    if not ClientViewModel then
        print("[Rivals SC] ClientViewModel not ready yet. Will retry on next character spawn.")
        _G.rivalsSCHooked = false
        return
    end
    
    _G.RivalsClientViewModel = ClientViewModel

    if not CosmeticLibrary or not ClientViewModel or not ReplicatedClass then
        FlashStatus("Module load failed.", Color3.fromRGB(220, 80, 80))
        _G.rivalsSCHooked = false
        return
    end

    local function makeCosmeticData(name, cType)
    ...
    -- Always restore to the true original before patching.
    -- If we previously patched (stored in _G), restore first so we never
    -- double-wrap — double-wrapping leaves stale closure upvalues pointing
    -- into an old Lua VM after teleport, which causes a hard crash.
    if _G.rivalsOrigGetWrap and _G.rivalsCVMRef then
        pcall(function() _G.rivalsCVMRef.GetWrap = _G.rivalsOrigGetWrap end)
    end
    if _G.rivalsOrigNew and _G.rivalsCVMRef then
        pcall(function() _G.rivalsCVMRef.new = _G.rivalsOrigNew end)
    end

    -- Save the originals from the CURRENT module (not cached)
    _G.rivalsOrigGetWrap = ClientViewModel.GetWrap
    _G.rivalsOrigNew     = ClientViewModel.new
    _G.rivalsCVMRef      = ClientViewModel

    local _origGetWrap = _G.rivalsOrigGetWrap
    ClientViewModel.GetWrap = function(self)
        -- Bail out if we've shut down, so the patch can't fire after teleport
        if not _G.rivalsActive then return _origGetWrap(self) end
        local ok, res = pcall(function()
            local wName = _G.rivalsEquipped[self.ClientItem and self.ClientItem.Name]
            wName = wName and wName.Wrap
            if wName and wName ~= "None" then return makeCosmeticData(wName, "Wrap") end
        end)
        if ok and res then return res end
        return _origGetWrap(self)
    end

    local _origNew = _G.rivalsOrigNew
    ClientViewModel.new = function(replicatedData, clientItem)
        -- Bail out if we've shut down, so the patch can't fire after teleport
        if not _G.rivalsActive then return _origNew(replicatedData, clientItem) end
        if clientItem then
            local equipped = _G.rivalsEquipped[clientItem.Name]
            if equipped then
                pcall(function()
                    local cf = rawget(clientItem, "ClientFighter")
                    if not cf then
                        local ok2, v = pcall(function() return clientItem.ClientFighter end)
                        if ok2 then cf = v end
                    end
                    ...
                    -- Inject Skin
                    if equipped.Skin and equipped.Skin ~= "Default" then
                        local cosData = makeCosmeticData(equipped.Skin, "Skin")
                        if cosData then
                            local skinKey = ReplicatedClass:ToEnum("Skin")
                            replicatedData[dataKey][skinKey] = cosData
                            replicatedData[dataKey][nameKey] = equipped.Skin
                        end
                    end
                    
                    -- Inject Charm
                    if equipped.Charm and equipped.Charm ~= "None" then
                        local charmData = makeCosmeticData(equipped.Charm, "Charm")
                        if charmData then
                            local charmKey = ReplicatedClass:ToEnum("Charm")
                            replicatedData[dataKey][charmKey] = charmData
                        end
                    end
                    
                    -- Inject Finisher (if enum exists)
                    if equipped.Finisher and equipped.Finisher ~= "None" then
                        local finisherData = makeCosmeticData(equipped.Finisher, "Finisher")
                        if finisherData then
                            -- Try to get Finisher enum key
                            local success, finisherKey = pcall(function()
                                return ReplicatedClass:ToEnum("Finisher")
                            end)
                            if success and finisherKey then
                                replicatedData[dataKey][finisherKey] = finisherData
                            end
                        end
                    end
                end)
        ...
        return _origNew(replicatedData, clientItem)
    end

    print("[Rivals SC] Hooks applied successfully to ClientViewModel")

    -- FlashStatus might not exist yet if GUI hasn't been built (e.g., 1v1 place on startup)
    pcall(function()
        if FlashStatus then
            FlashStatus("Modules loaded. Ready.", Theme.Accent)
        end
    end)
    print("[Rivals SC] Ready.")
    
    -- Load wraps, charms, and finishers from CosmeticLibrary
    task.spawn(function()
        pcall(function()
            if not CosmeticLibrary or not CosmeticLibrary.Cosmetics then return end
            ...
            for name, data in pairs(CosmeticLibrary.Cosmetics) do
                if data.Type == "Charm" then
                    table.insert(charms, name)
                elseif data.Type == "Finisher" then
                    table.insert(finishers, name)
                elseif data.Type == "Wrap" then
                    table.insert(wraps, name)
                end
            end
            ...
            _charmsList = charms
            _finishersList = finishers
            _wrapsList = wraps
            
            print("[Rivals SC] Loaded " .. #charms .. " charms, " .. #finishers .. " finishers, " .. #wraps .. " wraps.")
        end)
    end)
end

-- Misc tab: Unlock Skin Changer button (user must unlock before hooks apply)
requestUnlockSkinChanger = function()
    local reapply = _G.rivalsSCHooked == true or _G.rivalsSkinChangerUnlocked == true
    _G.rivalsSkinChangerUnlocked = true
    -- Force a clean apply / re-apply of ClientViewModel patches
    _G.rivalsSCHooked = false
    _G.rivalsHookAttempted = false
    FlashStatus(reapply and "Re-applying skin hooks…" or "Unlocking skin changer…", Theme.Accent)
    runHooks()
    pcall(refreshUnlockStatus)
    if _G.rivalsSCHooked then
        FlashStatus("Skin changer unlocked.", Color3.fromRGB(80, 220, 120))
    else
        FlashStatus("Unlock failed — modules not ready. Try in a match/lobby.", Color3.fromRGB(220, 80, 80))
    end
end
pcall(refreshUnlockStatus)
...
-- Initial load: only auto-hook if user already unlocked (re-execute / same session)
task.spawn(function()
    if not _G.rivalsSkinChangerUnlocked then
        FlashStatus("Skin changer locked — open Misc tab to unlock.", Theme.Dim)
        pcall(refreshUnlockStatus)
        return
    end
    if not _G.rivalsHookAttempted then
        if waitForFullLoad() then
            _G.rivalsHookAttempted = true
            runHooks()
            pcall(refreshUnlockStatus)
        end
    end
...
-- Re-hook on character spawn only after user has unlocked once.
pcall(function()
...
-- ============================================================
-- ANTICHEAT BYPASS
-- Disables LocalScript/ModuleScript whose name matches known
-- Rivals anticheat patterns on initial scan only.
-- NOTE: No DescendantAdded watcher and no NetworkClient tampering —
-- those fire on game-critical objects during matchmaking and crash.
-- ============================================================
local AC_PATTERNS = {
    "anticheat", "anti_cheat", "anti cheat", "cheatdetect",
    "sanitycheck", "sanity_check", "exploit", "detectexploit",
    "kickhandler", "kickplayer", "banhandler", "remotespy",
    "integritycheck", "integrity_check", "flagcheck",
}
...
local function tryDisable(obj)
    pcall(function()
        if (obj:IsA("LocalScript") or obj:IsA("ModuleScript")) and isAnticheatName(obj.Name) then
            obj.Disabled = true
        end
    end)
end

-- One-time scan only — no live watcher, no NetworkClient hooks
pcall(function()
...
-- ============================================================
-- "ANTICHEAT BYPASSED" NOTIFICATION  (bottom-right toast)
-- ============================================================
local function showBypassNotification()
    pcall(function()
        local pg = player:FindFirstChild("PlayerGui")
        if not pg then return end
        local existing = pg:FindFirstChild("__rivalsACBypassToast")
        if existing then existing:Destroy() end

        local toastSG = Instance.new("ScreenGui")
        toastSG.Name = "__rivalsACBypassToast"
        toastSG.ResetOnSpawn = false
        toastSG.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
        toastSG.Parent = pg

        local toast = Instance.new("Frame", toastSG)
        toast.Size = UDim2.new(0, 220, 0, 40)
        toast.Position = UDim2.new(1, -230, 1, -60)
        toast.BackgroundColor3 = Theme.Main
        ...
        Instance.new("UICorner", dot).CornerRadius = UDim.new(0, 99)

        local lbl = Instance.new("TextLabel", toast)
        lbl.Size = UDim2.new(1, -30, 1, 0)
        lbl.Position = UDim2.new(0, 26, 0, 0)
        lbl.BackgroundTransparency = 1
        lbl.Text = "anticheat bypassed!"
        ...
        TW:Create(toast, TweenInfo.new(0.3), {BackgroundTransparency = 0}):Play()
        TW:Create(lbl,   TweenInfo.new(0.3), {TextTransparency = 0}):Play()
        TW:Create(stroke,TweenInfo.new(0.3), {Transparency = 0}):Play()
        TW:Create(dot,   TweenInfo.new(0.3), {BackgroundTransparency = 0}):Play()

        -- Fade out and destroy after 3.5 seconds
        task.delay(3.5, function()
            pcall(function()
                TW:Create(toast, TweenInfo.new(0.4), {BackgroundTransparency = 1}):Play()
                TW:Create(lbl,   TweenInfo.new(0.4), {TextTransparency = 1}):Play()
                TW:Create(stroke,TweenInfo.new(0.4), {Transparency = 1}):Play()
                TW:Create(dot,   TweenInfo.new(0.4), {BackgroundTransparency = 1}):Play()
                task.delay(0.5, function()
                    pcall(function() toastSG:Destroy() end)
                end)
            end)
        end)
    end)
...
showBypassNotification()


-- ============================================================
-- MCP INTEGRATION API
-- Expose functions for external control via MCP
-- ============================================================
_G.RivalsAPI = _G.RivalsAPI or {}
_G.RivalsAPI.version = "4.1.3"

-- Get all available cosmetics
function _G.RivalsAPI.GetCosmetics()
    return {
        charms = _charmsList or {},
        finishers = _finishersList or {},
        wraps = _wrapsList or {},
        weapons = (function()
            local w = {}
            for weapon in pairs(SkinLists) do
                table.insert(w, weapon)
            end
            table.sort(w)
            return w
        end)()
...
function _G.RivalsAPI.GetEquipped()
    local equipped = {}
    for weapon, info in pairs(_G.rivalsEquipped) do
        equipped[weapon] = {
            skin = info.Skin,
...
function _G.RivalsAPI.EquipSkin(weapon, skinName)
    if not _G.rivalsEquipped[weapon] then
        return false, "Invalid weapon"
    end
    if not SkinLists[weapon] then
    ...
    if not found and skinName ~= "Default" then
        return false, "Skin not found for weapon"
    end
    
    _G.rivalsEquipped[weapon].Skin = skinName
    return true, "Equipped " .. skinName .. " on " .. weapon
end

-- Equip a wrap for a weapon
function _G.RivalsAPI.EquipWrap(weapon, wrapName)
    if not _G.rivalsEquipped[weapon] then
        return false, "Invalid weapon"
    end
    
    if wrapName ~= "None" and _wrapsList then
        local found = false
        for _, wrap in ipairs(_wrapsList) do
            if wrap == wrapName then
                found = true
                break
            end
        end
        if not found then
            return false, "Wrap not found"
        end
    end
    
    _G.rivalsEquipped[weapon].Wrap = wrapName
    return true, "Equipped wrap " .. wrapName .. " on " .. weapon
end

-- Equip a charm for a weapon
function _G.RivalsAPI.EquipCharm(weapon, charmName)
    if not _G.rivalsEquipped[weapon] then
        return false, "Invalid weapon"
    end
    
    if charmName ~= "None" and _charmsList then
        local found = false
        for _, charm in ipairs(_charmsList) do
            if charm == charmName then
                found = true
                break
            end
        end
    ...
    _G.rivalsEquipped[weapon].Charm = charmName
    return true, "Equipped charm " .. charmName .. " on " .. weapon
end

-- Equip a finisher for a weapon
function _G.RivalsAPI.EquipFinisher(weapon, finisherName)
    if not _G.rivalsEquipped[weapon] then
        return false, "Invalid weapon"
    end
    
    if finisherName ~= "None" and _finishersList then
        local found = false
        for _, finisher in ipairs(_finishersList) do
            if finisher == finisherName then
                found = true
                break
            end
        end
        if not found then
            return false, "Finisher not found"
        end
    end
    
    _G.rivalsEquipped[weapon].Finisher = finisherName
    _G.rivalsGlobalFinisher = finisherName
    
    -- Apply via remote
    pcall(function()
        local RS = game:GetService("ReplicatedStorage")
        local equipRemote = RS.Remotes.Data.EquipCosmetic
        if equipRemote then
            equipRemote:FireServer(finisherName, "Finisher")
        end
    end)
    
    return true, "Equipped finisher " .. finisherName .. " on " .. weapon
end

-- Get skin preview data
function _G.RivalsAPI.GetPreview(itemName)
    local data = GetSkinData(itemName)
    if data then
        return {
            name = itemName,
            image = data.Image,
...
function _G.RivalsAPI.ToggleMenu()
    menuVisible = not menuVisible
    win.Visible = menuVisible
    return menuVisible
end

function _G.RivalsAPI.OpenMenu()
    menuVisible = true
    win.Visible = true
    return true
end

function _G.RivalsAPI.CloseMenu()
    menuVisible = false
    win.Visible = false
    return false
end

_G.OpenRivalsSkinChanger = _G.RivalsAPI.OpenMenu
_G.CloseRivalsSkinChanger = _G.RivalsAPI.CloseMenu
_G.ToggleRivalsSkinChanger = _G.RivalsAPI.ToggleMenu

-- Save configuration
function _G.RivalsAPI.SaveConfig()
    return SaveConfig()
end

-- Load configuration
function _G.RivalsAPI.LoadConfig()
    return LoadConfig()
end

print("[Rivals SC] MCP API exposed via _G.RivalsAPI")

print("[instance] skin changer ready — character > weapon skins, or K")

]==]
    local fn, err = loadstring(src)
    if not fn then
        warn("[newrivals2] Skin changer loadstring failed: " .. tostring(err))
        return
    end
    local ok, runErr = pcall(fn)
    if not ok then
        warn("[newrivals2] Skin changer runtime error: " .. tostring(runErr))
    end
end)
